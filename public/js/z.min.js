...
"dms");a.direct_message.sender.screen_name!=screen_name&&z_engine_notification(a.direct_message.sender.profile_image_url,"@"+a.direct_message.sender.screen_name+" sent a direct message",a.direct_message.text)}else if(a.dms){dms_loaded++;a.dms.each(function(g){z_engine_tweet(g,"dms")});switch(dms_loaded){case 1:$("loading-inbox").fade();$("loading-outbox").appear();break;case 2:$("loading-outbox").fade()}}else if(a.event)switch(a.event){case "block":$w(store.get("mention_blocks")).uniq();c=$w(store.get("user_blocks")).uniq();
var h=false,j=false;d.each(function(g){if(g==a.target.screen_name)h=true});c.each(function(g){if(g==a.target.screen_name)j=true});h||store.set("mention_blocks",current_blocks.join(" ")+" "+a.target.screen_name);j||store.set("user_blocks",current_blocks.join(" ")+" "+a.target.screen_name);break;case "unblock":d=$w(store.get("user_blocks")).uniq();c=$w(store.get("user_blocks")).uniq();var e="",f="";d.each(function(g){if(g!=a.target.screen_name)e+=g+" "});c.each(function(g){if(g!=a.target.screen_name)f+=
g+" "});store.set("mention_blocks",e);store.set("user_blocks",f);break;case "favorite":a.source.screen_name!=screen_name&&z_engine_notification(a.source.profile_image_url,"@"+a.source.screen_name+" faved your tweet!",a.target_object.text);break;case "unfavorite":break;case "follow":a.source.screen_name!=screen_name&&z_engine_notification(a.source.profile_image_url,"@"+a.source.screen_name+" is following you!",a.source.description);break;case "list_member_added":a.source.screen_name!=screen_name&&
...
n=false;v=b.text;y=b.sender.statuses_count;A=b.sender_id;q=b.sender.verified}var H=false,I=false,J=false,K=false,r=false;if(a!="threaded"&&content_stored[i])r=true;a!="dms"&&$w(store.get("client_blocks")).uniq().each(function(w){if(w.replace("'","")==E)H=true});if(f){typeof f.hashtags=="object"&&$w(store.get("hashtag_blocks")).uniq().each(function(w){f.hashtags.uniq().each(function(F){if(w==F)I=true})});typeof f.user_mentions=="object"&&$w(store.get("mention_blocks")).uniq().each(function(w){f.user_mentions.uniq().each(function(F){if(w==
