...
var i=false,j=false;d.each(function(h){if(h==a.target.screen_name)i=true});c.each(function(h){if(h==a.target.screen_name)j=true});i||store.set("mention_blocks",current_blocks.join(" ")+" "+a.target.screen_name);j||store.set("user_blocks",current_blocks.join(" ")+" "+a.target.screen_name);break;case "unblock":d=$w(store.get("user_blocks")).uniq();c=$w(store.get("user_blocks")).uniq();var e="",f="";d.each(function(h){if(h!=a.target.screen_name)e+=h+" "});c.each(function(h){if(h!=a.target.screen_name)f+=
h+" "});store.set("mention_blocks",e);store.set("user_blocks",f);break;case "favorite":a.source.screen_name!=screen_name&&z_engine_notification(a.source.profile_image_url,"@"+a.source.screen_name+" faved your tweet!",a.target_object.text);break;case "unfavorite":break;case "follow":a.source.screen_name!=screen_name&&z_engine_notification(a.source.profile_image_url,"@"+a.source.screen_name+" is following you!",a.source.description);break;case "list_member_added":a.source.screen_name!=screen_name&&
...
function z_engine_set_klout(b,a){if(typeof b=="object")if(b.length>0&&typeof a=="string"){var c=Math.round(b[0].score.network_score),d=Math.round(b[0].score.true_reach),i="score: <strong>"+Math.round(b[0].score.kscore)+"</strong><br />";i+="amp: <strong>"+Math.round(b[0].score.amplification_score)+"</strong><br />";i+="network: <strong>"+c+"</strong><br />";i+="reach: <strong>"+d+"</strong><br />";if($("klout-"+a)){$("klout-"+a).addTip(i,kloutinfo_params);$("klout-"+a).setAttribute("src","img/kltd.png");
$("klout-"+a).setAttribute("title","")}if($("klout-"+a+"-mentioned")){$("klout-"+a+"-mentioned").addTip(i,kloutinfo_params);$("klout-"+a+"-mentioned").setAttribute("src","img/kltd.png");$("klout-"+a+"-mentioned").setAttribute("title","")}}}function z_engine_shorten_urls(){var b=$("new-tweet").getValue();b.length>0&&b.replace(/((https?\:\/\/)|(www\.))([^ ]+)/g,function(a){socket.emit("message",{shorten:a})})}
...
function z_engine_tweet(b,a){if(a!="dms")if(b.retweeted_status){c=b.retweeted_status.user.screen_name;d=b.user.screen_name;i=b.retweeted_status.user.profile_image_url;j=(new Date(b.retweeted_status.created_at)).toLocaleString().replace(/GMT.+/,"");e=b.retweeted_status.user.description;f=b.retweeted_status.entities;h=b.retweeted_status.favorited;l=b.retweeted_status.user.followers_count;s=b.retweeted_status.user.friends_count;g=b.retweeted_status.id_str;o=b.retweeted_status.user.location;z=b.retweeted_status.user["protected"];
p=b.retweeted_status.place;m=b.retweeted_status.in_reply_to_screen_name;x=b.retweeted_status.in_reply_to_status_id_str;n=true;E=b.retweeted_status.source.stripTags();v=b.retweeted_status.text;y=b.retweeted_status.user.statuses_count;A=b.retweeted_status.user.id;q=b.retweeted_status.user.verified}else var c=b.user.screen_name,d=false,i=b.user.profile_image_url,j=(new Date(b.created_at)).toLocaleString().replace(/GMT.+/,""),e=b.user.description,f=b.entities,h=b.favorited,l=b.user.followers_count,s=
b.user.friends_count,g=b.id_str,o=b.user.location,z=b.user["protected"],p=b.place,m=b.in_reply_to_screen_name,x=b.in_reply_to_status_id_str,n=false,E=b.source.stripTags(),v=b.text,y=b.user.statuses_count,A=b.user.id,q=b.user.verified;else{c=b.sender.screen_name;i=b.sender.profile_image_url;j=(new Date(b.created_at)).toLocaleString().replace(/GMT.+/,"");e=b.sender.description;f=false;l=b.sender.followers_count;s=b.sender.friends_count;g=b.id_str;o=b.sender.location;z=false;m=b.recipient.screen_name;
n=false;v=b.text;y=b.sender.statuses_count;A=b.sender_id;q=b.sender.verified}var H=false,I=false,J=false,K=false,r=false;if(a!="threaded"&&content_stored[g])r=true;a!="dms"&&$w(store.get("client_blocks")).uniq().each(function(w){if(w.replace("'","")==E)H=true});if(f){typeof f.hashtags=="object"&&$w(store.get("hashtag_blocks")).uniq().each(function(w){f.hashtags.uniq().each(function(F){if(w==F)I=true})});typeof f.user_mentions=="object"&&$w(store.get("mention_blocks")).uniq().each(function(w){f.user_mentions.uniq().each(function(F){if(w==
F)J=true})})}$w(store.get("user_blocks")).uniq().each(function(w){if(w==c)K=true});if(!r&&!H&&!I&&!J&&!K){content_stored[g]=JSON.stringify(b);r="";if(e!=null)r+=e+"<br /><br />";if(o!=null)r+="location: "+o+"<br />";r+="tweets: <strong>"+y+"</strong><br />";r+="following: <strong>"+s+"</strong><br />";r+="followers: <strong>"+l+"</strong>";if(f){o=z_engine_tweet_mentioned(f);e=z_engine_tweet_mentioned_string(f)}else e=o=false;var D=new Element("br");l=new Element("div",{id:"comment-"+g,"class":"comment-parent"});
y=new Element("div",{"class":"comment-profile-wrapper left"});var B=new Element("div",{"class":"comment-gravatar"});s=new Element("img",{id:"av-"+g,src:i,style:"height: 50px; width: 50px; cursor: pointer;",alt:""});B.insert(s);y.insert(B);B=new Element("div",{id:"comment-"+g+"content","class":"comment-content-wrapper right"});var C=c!=screen_name?o?new Element("div",{"class":"comment-body-mentioned"}):new Element("div",{"class":"comment-body"}):new Element("div",{"class":"comment-body-me"}),k=new Element("div",
{"class":"comment-arrow"});C.insert(k);var t=new Element("div",{"class":"post-date"});k=new Element("div",{"class":"left",id:"left-"+g});var L=new Element("a",{target:"_blank",href:"http://twitter.com/"+c});L.update("@"+c+" ");var M=new Element("span");M.update("wrote this ");if(a!="dms")var u=new Element("a",{target:"_blank",id:"comment-"+g+"-relative-date",href:"http://twitter.com/"+c+"/status/"+g});var G=new Element("time",{datetime:j,"class":a});G.update(relative_time(j));a!="dms"&&u.insert(G);
...
g+"-text"});p=new Element("div",{id:"right-"+g,"class":"right"});t.insert(D);t.insert({bottom:p});D=new Element("div",{id:"text-"+g});D.update(z_engine_parse_tweet(v));t.insert({bottom:D});n.insert(t);C.insert({bottom:n});B.insert(C);l.insert(y);l.insert({bottom:B});l.insert({bottom:d});new Element.extend(s);new Element.extend(k);new Element.extend(p);new Element.extend(l);switch(a){case "dms":if(c!=screen_name)$("dms-inbox-timeline").insert({top:l});else c==screen_name&&$("dms-outbox-timeline").insert({top:l});
break;case "home":$("home-timeline").insert({top:l});break;case "mentions":$("mentions-timeline").insert({bottom:l})}if(o&&a!="mentions"&&a!="threaded"&&c!=screen_name&&!b.retweeted_status){o=cloneNodeWithEvents(l);o.setAttribute("id","comment-"+g+"-mentioned");k.setAttribute("id","left-"+g+"-mentioned");p.setAttribute("id","right-"+g+"-mentioned");s.setAttribute("id","av-"+g+"-mentioned");new Element.extend(o);$("mentions-timeline").insert({top:o});z_engine_tweet_buttons("mentions",g,c,A,v,z,h,e,
r);z_engine_notification(i,"@"+c+" mentioned you!",v);z_engine_fade_down("comment-"+g+"-mentioned")}if(a=="threaded"){k.setAttribute("id","left-"+g+"-threaded");p.setAttribute("id","right-"+g+"-threaded");s.setAttribute("id","av-"+g+"-threaded");i=cloneNodeWithEvents(l);i.setAttribute("id","comment-"+g+"-threaded");new Element.extend(i);$("threaded-timeline").insert({bottom:i});z_engine_tweet_buttons("threaded",g,c,A,v,z,h,e,r);z_engine_fade_down("comment-"+g+"-threaded")}$("comment-"+g)&&z_engine_tweet_buttons(a,
g,c,A,v,z,h,e,r)}a=="threaded"&&x&&z_engine_threaded(false,x)}
function z_engine_tweet_buttons(b,a,c,d,i,j,e,f,h){i=(new Element("textarea")).update(escape_string(i.replace(/</g,"&lt;").replace(/>/g,"&gt;"))).getValue();switch(b){case "dms":$("av-"+a)&&$("av-"+a).addTip(z_engine_parse_tweet(h),userinfo_params);if(c!=screen_name){f=new Element("img",{onclick:'z_engine_reply_dm("'+d+'", "'+c+'");',src:"img/rep.png",id:"reply-"+a,alt:""});new Element.extend(f);$("right-"+a).insert(f)}e=new Element("img",{onclick:'z_engine_destroy("'+a+'", "dm");',src:"img/del.png",
id:"del-"+a,alt:""});new Element.extend(e);$("right-"+a).insert({bottom:e});break;case "home":$("av-"+a)&&$("av-"+a).addTip(z_engine_parse_tweet(h),userinfo_params);if($("left-"+a)&&!$("klout-"+a)){b=new Element("span",{"class":"klout"});b.update(" ");d=new Element("img",{onclick:'z_engine_get_klout("'+c+'", "'+d+'", "'+a+'");',src:"img/klt.png",id:"klout-"+a,alt:"",title:"click to get this users klout score"});b.insert({top:d});new Element.extend(d);$("left-"+a).insert({top:b})}if(c!=screen_name){f=
new Element("img",{src:"img/rep.png",onclick:'z_engine_reply("'+c+'", "'+a+'", "'+f+'");',id:"reply-"+a,alt:""});new Element.extend(f);if(j)c=new Element("img",{src:"img/lock.png",style:"cursor: default;",alt:""});else{c=new Element("img",{src:"img/rt.png",onclick:'z_engine_retweet("'+a+'", "'+c+'", "'+i+'");',id:"rt-"+a,alt:""});new Element.extend(c)}e=e?new Element("img",{src:"img/favd.png",onclick:'z_engine_unfavorite("'+a+'");',id:"fave-"+a,alt:""}):new Element("img",{src:"img/fav.png",onclick:'z_engine_favorite("'+
a+'");',id:"fave-"+a,alt:""});new Element.extend(e);$("right-"+a).update();$("right-"+a).insert(f);$("right-"+a).insert({bottom:c})}else{e=new Element("img",{onclick:'z_engine_destroy("'+a+'", "tweet");',src:"img/del.png",id:"del-"+a,alt:""});new Element.extend(e);$("right-"+a).update()}$("right-"+a).insert({bottom:e});break;case "mentions":$("av-"+a+"-mentioned")&&$("av-"+a+"-mentioned").addTip(z_engine_parse_tweet(h),userinfo_params);if($("left-"+a+"-mentioned")&&!$("klout-"+a+"-mentioned")){b=
...
a+"-mentioned")){$("right-"+a+"-mentioned").update();$("right-"+a+"-mentioned").insert({bottom:e})}}break;case "threaded":$("av-"+a+"-threaded")&&$("av-"+a+"-threaded").addTip(z_engine_parse_tweet(h),userinfo_params);if(c!=screen_name){f=new Element("img",{src:"img/rep.png",onclick:'z_engine_reply("'+c+'", "'+a+'", "'+f+'");',id:"reply-"+a+"-threaded",alt:""});new Element.extend(f);if(j)c=new Element("img",{src:"img/lock.png",style:"cursor: default;",alt:""});else{c=new Element("img",{src:"img/rt.png",
onclick:'z_engine_retweet("'+a+'", "'+c+'", "'+i+'");',id:"rt-"+a+"-threaded",alt:""});new Element.extend(c)}e=e?new Element("img",{src:"img/favd.png",onclick:'z_engine_unfavorite("'+a+'");',id:"fave-"+a+"-threaded",alt:""}):new Element("img",{src:"img/fav.png",onclick:'z_engine_favorite("'+a+'");',id:"fave-"+a+"-threaded",alt:""});new Element.extend(e);if($("right-"+a+"-threaded")){$("right-"+a+"-threaded").update();$("right-"+a+"-threaded").insert(f);$("right-"+a+"-threaded").insert({bottom:c});
