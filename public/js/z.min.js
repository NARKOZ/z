...
{fetch:"dms-outbox"})},3E4);setInterval(function(){z_engine_update_relative_time("time.home")},update_relative_home_interval);setInterval(function(){z_engine_update_relative_time("time.mentions")},update_relative_mentions_interval);setInterval(function(){z_engine_update_relative_time("time.dms")},update_relative_dms_interval);setInterval(function(){paused||z_engine_prune_tweets()},prune_tweets_interval);setInterval(function(){paused||z_engine_stream_queue()},stream_queue_interval)}else if(b.account)store.set("account",
b.account);else if(b["delete"]){var e=typeof b["delete"].status=="object"?b["delete"].status.id_str:0;z_engine_drop_tweet(e)}else if(b.direct_message){z_engine_tweet(b.direct_message,"dms");b.direct_message.sender.screen_name!=screen_name&&z_engine_notification(b.direct_message.sender.profile_image_url,"@"+b.direct_message.sender.screen_name+" sent a direct message",b.direct_message.text)}else if(b.dms){dms_loaded++;b.dms.each(function(i){z_engine_tweet(i,"dms")});switch(dms_loaded){case 1:$("loading-inbox").fade();
$("loading-outbox").appear();break;case 2:$("loading-outbox").fade()}}else if(b.event)switch(b.event){case "block":e=$w(store.get("user_blocks")).uniq();var j=false;e.each(function(i){if(i==b.target.screen_name)j=true});if(!j){d=b.target.screen_name;e=e.join(" ")+" "+d;store.set("user_blocks",e)}break;case "unblock":e=$w(store.get("user_blocks")).uniq();var f="";e.each(function(i){if(i!=b.target.screen_name)f+=i+" "});store.set("user_blocks",f);break;case "favorite":b.source.screen_name!=screen_name&&
...
b.friends.join(" ");store.set("friends",e)}else if(b.home){b.home.each(function(i){z_engine_tweet(i,"home")});socket.emit("message",{fetch:"userstream"});$("loading-home").fade();$("loading-mentions").appear()}else if(b.imgur){if(b.imgur!="error"){d=b.imgur.upload.links.original;var h=$("new-tweet").getValue();if(h.length>0)e=h+" "+d;else if(h.length==0)e=d;$("new-tweet").setValue(e);$("image").setStyle("border-color: green;")}else $("image").setStyle("border-color: red;");setTimeout(function(){$("image").setStyle("border-color: #ddd;")},
1500)}else if(b.info){screen_name=b.info.screen_name;user_id=b.info.user_id;store.set("screen_name",screen_name);store.set("user_id",user_id)}else if(b.klout){d=b.klout;e=b.id_str;if(d!="error"||d.status!=200)z_engine_set_klout(d.users,e);else if($("klout-"+e)){$("klout-"+e).setStyle("cursor: default;");$("klout-"+e).setAttribute("onclick","");$("klout-"+e).addTip("<big><strong>?</strong></big>",{className:"klout",stem:true,target:true,targetJoint:["left","top"],tipJoint:["right","bottom"]})}}else if(b.mentions){b.mentions.each(function(i){z_engine_tweet(i,
...
socket.disconnect();socket.connect();$("new-tweet").enable()}}else if(b.shorten){console.log(JSON.stringify(b.shorten));h=$("new-tweet").getValue();h.search(b.original)&&h.replace(b.original,b.shorten);$("new-tweet").setValue(h)}else if(b.text&&b.user&&b.created_at){content_queued.push(JSON.stringify(b));if(paused){$("paused-count").update("("+pttid+")");pttid++}b.retweeted_status&&b.retweeted_status.user.screen_name==screen_name&&b.user.screen_name!=screen_name&&z_engine_notification(b.user.profile_image_url,
"@"+b.user.screen_name+" retweeted you!",b.retweeted_status.text)}}else if(d.isJSON())d.evalJSON(true)?console.log("unknown error, output: "+d):console.log("json could not be properly evaluated, skipping");else console.log("not json, skipping")});socket.on("disconnect",function(){$("new-tweet").disable()})}
...
function z_engine_tweet(a,c){if(c!="dms")if(a.retweeted_status){b=a.retweeted_status.user.screen_name;d=a.user.screen_name;e=a.retweeted_status.user.profile_image_url;j=(new Date(a.retweeted_status.created_at)).toLocaleString().replace(/GMT.+/,"");f=a.retweeted_status.user.description;h=a.retweeted_status.entities;i=a.retweeted_status.favorited;w=a.retweeted_status.user.followers_count;p=a.retweeted_status.user.friends_count;g=a.retweeted_status.id_str;k=a.retweeted_status.user.location;A=a.retweeted_status.user["protected"];
q=a.retweeted_status.place;l=a.retweeted_status.in_reply_to_screen_name;o=a.retweeted_status.in_reply_to_status_id_str;n=true;D=a.retweeted_status.source.stripTags();x=a.retweeted_status.text;y=a.retweeted_status.user.statuses_count;B=a.retweeted_status.user.id;r=a.retweeted_status.user.verified}else var b=a.user.screen_name,d=false,e=a.user.profile_image_url,j=(new Date(a.created_at)).toLocaleString().replace(/GMT.+/,""),f=a.user.description,h=a.entities,i=a.favorited,w=a.user.followers_count,p=
a.user.friends_count,g=a.id_str,k=a.user.location,A=a.user["protected"],q=a.place,l=a.in_reply_to_screen_name,o=a.in_reply_to_status_id_str,n=false,D=a.source.stripTags(),x=a.text,y=a.user.statuses_count,B=a.user.id,r=a.user.verified;else{b=a.sender.screen_name;e=a.sender.profile_image_url;j=(new Date(a.created_at)).toLocaleString().replace(/GMT.+/,"");f=a.sender.description;h=false;w=a.sender.followers_count;p=a.sender.friends_count;g=a.id_str;k=a.sender.location;A=false;l=a.recipient.screen_name;
n=false;x=a.text;y=a.sender.statuses_count;B=a.sender_id;r=a.sender.verified}var G=false,H=false,I=false,J=false;c!="dms"&&$w(store.get("client_blocks")).uniq().each(function(u){if(u==D)G=true});if(h){typeof h.hashtags=="object"&&$w(store.get("hashtag_blocks")).uniq().each(function(u){h.hashtags.uniq().each(function(E){if(u==E)H=true})});typeof h.user_mentions=="object"&&$w(store.get("mention_blocks")).uniq().each(function(u){h.user_mentions.uniq().each(function(E){if(u==E)I=true})})}$w(store.get("user_blocks")).uniq().each(function(u){if(u==
b)J=true});if(!content[g]&&!G&&!H&&!I&&!J){var v="";if(f!=null)v+=f+"<br /><br />";if(k!=null)v+="location: "+k+"<br />";v+="tweets: <strong>"+y+"</strong><br />";v+="following: <strong>"+p+"</strong><br />";v+="followers: <strong>"+w+"</strong>";ids[tid]=g;content[g]=true;var C=new Element("br");if(h){p=z_engine_tweet_mentioned(h);f=z_engine_tweet_mentioned_string(h)}else f=p=false;k=new Element("li",{id:"comment-"+g,"class":"comment-parent",style:"display: none; opacity: 0;"});y=new Element("div",
...
c!="mentions"&&b!=screen_name&&!a.retweeted_status){p=cloneNodeWithEvents(k);p.setAttribute("id","comment-"+g+"-mentioned");m.setAttribute("id","left-"+g+"-mentioned");q.setAttribute("id","right-"+g+"-mentioned");w.setAttribute("id","av-"+g+"-mentioned");z_engine_tweet_buttons("mentions",g,b,B,x,A,i,f,v);new Element.extend(p);$("mentions-timeline").insert({top:p});z_engine_notification(e,"@"+b+" mentioned you!",x);new S2.FX.Parallel([new Effect.Appear("comment-"+g+"-mentioned",{duration:1.25,mode:"relative",
transition:"easeOutSine"}),new Effect.BlindDown("comment-"+g+"-mentioned",{duration:0.7,mode:"relative",transition:"easeOutSine"})],{duration:1.5})}z_engine_tweet_buttons(c,g,b,B,x,A,i,f,v);new S2.FX.Parallel([new Effect.Appear("comment-"+g,{duration:1.25,mode:"relative",transition:"easeOutSine"}),new Effect.BlindDown("comment-"+g,{duration:0.7,mode:"relative",transition:"easeOutSine"})],{duration:1.5})}tid++}
function z_engine_tweet_buttons(a,c,b,d,e,j,f,h,i){switch(a){case "home":if(b!=screen_name){a=new Element("img",{src:"img/rep.png",onclick:'z_engine_reply("'+b+'", "'+c+'", "'+h+'");',id:"reply-"+c,alt:""});if(j)e=new Element("img",{src:"img/lock.png",style:"cursor: default;",alt:""});else{e=new Element("img",{src:"img/rt.png",onclick:'z_engine_retweet("'+c+'", "'+b+'", "'+escape_string(e)+'");',id:"rt-"+c,alt:""});new Element.extend(e)}f=f?new Element("img",{src:"img/favd.png",onclick:'z_engine_unfavorite("'+
...
new Element("img",{onclick:'z_engine_get_klout("'+b+'", "'+c+'");',src:"img/klt.png",id:"klout-"+c,alt:"",title:"click to get this users klout score"});f.insert({top:b});$("left-"+c).insert({top:f});new Element.extend(b)}$("av-"+c)&&$("av-"+c).addTip(i,{className:"user",offset:[11,0],stem:true,target:true,targetJoint:["right","middle"],tipJoint:["left","middle"]});break;case "mentions":if(b!=screen_name){a=new Element("img",{src:"img/rep.png",onclick:'z_engine_reply("'+b+'", "'+c+'-mentioned", "'+
h+'");',id:"reply-"+c+"-mentioned",alt:""});if(j)e=new Element("img",{src:"img/lock.png",style:"cursor: default;",alt:""});else{e=new Element("img",{src:"img/rt.png",onclick:'z_engine_retweet("'+c+'", "'+b+'", "'+escape_string(e)+'");',id:"rt-"+c+"-mentioned",alt:""});new Element.extend(e)}f=f?new Element("img",{src:"img/favd.png",onclick:'z_engine_unfavorite("'+c+'");',id:"fave-"+c+"-mentioned",alt:""}):new Element("img",{src:"img/fav.png",onclick:'z_engine_favorite("'+c+'");',id:"fave-"+c+"-mentioned",
...
c+"-mentioned")&&$("right-"+c+"-mentioned").insert({bottom:f});new Element.extend(f)}$("av-"+c)&&$("av-"+c).addTip(i,{className:"user",offset:[11,0],stem:true,target:true,targetJoint:["right","middle"],tipJoint:["left","middle"]});$("av-"+c+"-mentioned")&&$("av-"+c+"-mentioned").addTip(i,{className:"user",offset:[11,0],stem:true,target:true,targetJoint:["right","middle"],tipJoint:["left","middle"]});break;case "dms":if(b!=screen_name){a=new Element("img",{onclick:'z_engine_reply_dm("'+d+'", "'+b+
'");',src:"img/rep.png",id:"reply-"+c,alt:""});new Element.extend(a);$("right-"+c).insert(a)}f=new Element("img",{onclick:'z_engine_destroy("'+c+'", "dm");',src:"img/del.png",id:"del-"+c,alt:""});new Element.extend(f);$("right-"+c).insert({bottom:f})}}
