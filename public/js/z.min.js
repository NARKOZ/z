...
minChars:2});store.get("geo")=="on"&&new HotKey("l",function(){z_engine_geo()},{shiftKey:true});new HotKey("o",function(){z_engine_clear_timeline()},{shiftKey:true});new HotKey("p",function(){z_engine_tweet_pause()},{shiftKey:true});new HotKey("s",function(){z_engine_shorten_urls()},{shiftKey:true});new Control.Modal($(document.body).down("[href=/account/settings]"),{className:"window",closeOnClick:"overlay",fade:true,method:"GET",overlayOpacity:0.5});new Event.observe("new-tweet","keyup",function(){z_engine_input()});
...
d=$w(store.get("user_blocks")).uniq().strip();var i=false,k=false;b.each(function(f){if(f==a.target.screen_name)i=true});d.each(function(f){if(f==a.target.screen_name)k=true});i||store.set("mention_blocks",current_blocks.join(" ")+" "+a.target.screen_name);k||store.set("user_blocks",current_blocks.join(" ")+" "+a.target.screen_name);break;case "unblock":b=$w(store.get("user_blocks")).uniq().strip();d=$w(store.get("user_blocks")).uniq().strip();var l="",n="";b.each(function(f){if(f!=a.target.screen_name)l+=
f+" "});d.each(function(f){if(f!=a.target.screen_name)n+=f+" "});store.set("mention_blocks",l);store.set("user_blocks",n);break;case "favorite":if(a.source.screen_name!=screen_name){b=a.source.profile_image_url;d=a.target_object.text;e="@"+a.source.screen_name+" faved your tweet!";z_engine_notification(b,e,d)}break;case "unfavorite":break;case "follow":if(a.source.screen_name!=screen_name){b=a.source.profile_image_url;d=a.source.description;e="@"+a.source.screen_name+" is following you!";z_engine_notification(b,
