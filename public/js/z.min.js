...
"notice!","error occurred, reconnecting...")}});socket.on("shorten",function(a){a=$("new-tweet").getValue().replace(a.original,a.shorten);$("new-tweet").setValue(a)});socket.on("show",function(a){content_stored[a.retweeted_status?a.retweeted_status.id_str:a.id_str]=JSON.stringify(a);z_engine_tweet(a,"threaded bottom");rates--});socket.on("tweet",function(a){if(a.retweeted_status&&a.retweeted_status.user.screen_name==screen_name&&a.user.screen_name!=screen_name){var b=a.retweeted_status.retweet_count-
1;z_engine_notification(a.user.profile_image_url,b>1?"@"+a.user.screen_name+" and "+b+" others rt'd you!":b==1?"@"+a.user.screen_name+" and 1 other rt'd you!":"@"+a.user.screen_name+" retweeted you!",a.retweeted_status.text)}else{content_queued.push(JSON.stringify(a));if(paused){$("paused-count").update("("+pttid+")");pttid++}}});socket.on("disconnect",function(){$("new-tweet").disable()})}else $("new-tweet").setValue("sorry, your browser may not support this client!")}
