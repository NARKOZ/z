...
function z_engine_tweet(a,b){if(b!="dms")if(a.retweeted_status){c=a.retweeted_status.user.screen_name;d=a.user.screen_name;j=a.retweeted_status.user.profile_image_url;h=(new Date(a.retweeted_status.created_at)).toLocaleString().replace(/GMT.+/,"");i=a.retweeted_status.user.description;f=a.retweeted_status.entities;t=a.retweeted_status.favorited;g=a.retweeted_status.user.followers_count;k=a.retweeted_status.user.friends_count;e=a.retweeted_status.id_str;n=a.retweeted_status.user.location;z=a.retweeted_status.user["protected"];
p=a.retweeted_status.place;m=a.retweeted_status.in_reply_to_screen_name;x=a.retweeted_status.in_reply_to_status_id_str;o=a.retweeted_status.retweet_count;q=true;E=a.retweeted_status.source.stripTags();v=a.retweeted_status.text;y=a.retweeted_status.user.statuses_count;A=a.retweeted_status.user.id;r=a.retweeted_status.user.verified}else var c=a.user.screen_name,d=false,j=a.user.profile_image_url,h=(new Date(a.created_at)).toLocaleString().replace(/GMT.+/,""),i=a.user.description,f=a.entities,t=a.favorited,
g=a.user.followers_count,k=a.user.friends_count,e=a.id_str,n=a.user.location,z=a.user["protected"],p=a.place,m=a.in_reply_to_screen_name,x=a.in_reply_to_status_id_str,o=a.retweet_count,q=false,E=a.source.stripTags(),v=a.text,y=a.user.statuses_count,A=a.user.id,r=a.user.verified;else{c=a.sender.screen_name;j=a.sender.profile_image_url;h=(new Date(a.created_at)).toLocaleString().replace(/GMT.+/,"");i=a.sender.description;f=false;g=a.sender.followers_count;k=a.sender.friends_count;e=a.id_str;n=a.sender.location;
z=false;m=a.recipient.screen_name;o=0;q=false;v=a.text;y=a.sender.statuses_count;A=a.sender_id;r=a.sender.verified}var I=false,J=false,K=false,L=false,s=false;if(b!="threaded"&&content_stored[e])s=true;b!="dms"&&$w(store.get("client_blocks")).uniq().each(function(w){if(w.replace("'","")==E)I=true});if(f){typeof f.hashtags=="object"&&$w(store.get("hashtag_blocks")).uniq().each(function(w){f.hashtags.uniq().each(function(F){if(w==F)J=true})});typeof f.user_mentions=="object"&&$w(store.get("mention_blocks")).uniq().each(function(w){f.user_mentions.uniq().each(function(F){if(w==
F)K=true})})}$w(store.get("user_blocks")).uniq().each(function(w){if(w==c)L=true});if(!s&&!I&&!J&&!K&&!L){(new Image).src=j;content_stored[e]=true;z_engine_remember_author(c);s="";if(i!=null)s+=i+"<br /><br />";if(n!=null)s+="location: "+n+"<br />";s+="tweets: <strong>"+y+"</strong><br />";s+="following: <strong>"+k+"</strong><br />";s+="followers: <strong>"+g+"</strong>";if(f){n=z_engine_tweet_mentioned(f);i=z_engine_tweet_mentioned_string(f)}else i=n=false;var D=new Element("br");g=new Element("div",
...
{"class":"comment-body-me"}),l=new Element("div",{"class":"comment-arrow"});C.insert(l);var G=new Element("div",{"class":"post-date"});l=new Element("div",{"class":"left",id:"left-"+e});var M=new Element("a",{target:"_blank",href:"http://twitter.com/"+c});M.update("@"+c+" ");var N=new Element("span");N.update("wrote this ");if(b!="dms")var u=new Element("a",{target:"_blank",id:"comment-"+e+"-relative-date",href:"http://twitter.com/"+c+"/status/"+e});var H=new Element("time",{datetime:h,"class":b});
H.update(relative_time(h));b!="dms"&&u.insert(H);l.insert(M);l.insert({bottom:N});if(b!="dms"){l.insert({bottom:u});if(x){h=new Element("span");h.update(" in reply to ");u=b=="threaded"?new Element("a",{target:"_blank",href:"http://twitter.com/"+m+"/status/"+x}):new Element("span",{onclick:'z_engine_threaded("'+e+'","'+x+'");',style:"cursor: pointer;"});u.update(m+" ");l.insert(h);l.insert({bottom:u})}m=new Element("span",{"class":"via"});m.update(" via "+E);l.insert({bottom:m})}else{l.insert({bottom:H});
if(m){h=new Element("span");h.update(" in reply to ");u=new Element("a",{target:"_blank",href:"http://twitter.com/"+m});u.update(m+" ");l.insert(h);l.insert({bottom:u})}}if(r){r=new Element("span");r.update(" ");m=new Element("img",{src:"img/ver.png",alt:"",title:"verified user"});r.insert({bottom:m});l.insert({bottom:r})}G.insert(l);r=new Element("div",{"class":"right"});if(q){q=o>0?"retweeted by "+d+" and "+o+" others!":"retweeted by "+d+"!";o=new Element("span",{"class":"rtd"});o.update(" ");q=
new Element("img",{src:"img/rtd2.png",alt:"",title:q});m=new Element("a",{target:"_blank",href:"http://twitter.com/"+d});m.update(d);o.insert({top:q});o.insert({bottom:m});r.insert({bottom:o})}if(b!="dms"&&p&&p.full_name){d=new Element("span",{"class":"place"});p=new Element("a",{target:"_blank",href:"http://maps.google.com?q="+p.full_name});o=new Element("img",{src:"img/plc.png",alt:""});p.update(o);d.update(p);r.insert({bottom:d})}G.insert({bottom:r});C.insert({bottom:G});d=new Element("div",{"class":"clearer"});
d.update("&nbsp;");C.insert({bottom:d});o=new Element("div",{"class":"comment-text"});q=new Element("p",{id:"comment-"+e+"-text"});p=new Element("div",{id:"right-"+e,"class":"right"});q.insert(D);q.insert({bottom:p});D=new Element("div",{id:"text-"+e});D.update(z_engine_parse_tweet(v));q.insert({bottom:D});o.insert(q);C.insert({bottom:o});B.insert(C);g.insert(y);g.insert({bottom:B});g.insert({bottom:d});new Element.extend(k);new Element.extend(l);new Element.extend(p);new Element.extend(g);switch(b){case "dms":if(c!=
screen_name)$("dms-inbox-timeline").insert({top:g});else c==screen_name&&$("dms-outbox-timeline").insert({top:g});break;case "home":$("home-timeline").insert({top:g});break;case "mentions":$("mentions-timeline").insert({top:g})}if(n&&b!="mentions"&&b!="threaded"&&c!=screen_name&&!a.retweeted_status){n=cloneNodeWithEvents(g);n.setAttribute("id","comment-"+e+"-mentioned");l.setAttribute("id","left-"+e+"-mentioned");p.setAttribute("id","right-"+e+"-mentioned");k.setAttribute("id","av-"+e+"-mentioned");
new Element.extend(n);$("mentions-timeline").insert({top:n});z_engine_tweet_buttons("mentions",e,c,A,v,z,t,i,s);z_engine_notification(j,"@"+c+" mentioned you!",v);z_engine_fade_down("comment-"+e+"-mentioned")}if(b=="threaded"){l.setAttribute("id","left-"+e+"-threaded");p.setAttribute("id","right-"+e+"-threaded");k.setAttribute("id","av-"+e+"-threaded");j=cloneNodeWithEvents(g);j.setAttribute("id","comment-"+e+"-threaded");new Element.extend(j);$("threaded-timeline").insert({bottom:j});z_engine_tweet_buttons("threaded",
e,c,A,v,z,t,i,s);z_engine_fade_down("comment-"+e+"-threaded")}if($("comment-"+e)){z_engine_tweet_buttons(b,e,c,A,v,z,t,i,s);z_engine_fade_down("comment-"+e)}}b=="threaded"&&x&&z_engine_threaded(false,x)}
