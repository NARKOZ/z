...
140)$("new-tweet").setStyle("color: #4d4d4d;");else $("new-tweet").getValue().length>=141&&$("new-tweet").setStyle("color: red;")});z_engine_geo();z_engine_image_dropper();z_engine_notification_setup()}socket.on("connect",function(){loaded||$("new-tweet").setValue("connected...")});socket.on("message",function(a){var b=JSON.stringify(a);if(b.isJSON()&&b.evalJSON(true)){b="";if(a.loaded&&!loaded){loaded=true;$("new-tweet").setValue("");$("new-tweet").enable();new Event.observe("new-tweet-form","submit",
function(f){Event.stop(f);z_engine_send_tweet()});z_engine_clicker("home-timeline-click","home-timeline");z_engine_clicker("mentions-timeline-click","mentions-timeline");z_engine_clicker("dms-inbox-timeline-click","dms-inbox-timeline");z_engine_clicker("dms-outbox-timeline-click","dms-outbox-timeline");z_engine_check_ratelimit();socket.emit("message",{fetch:"home"});setTimeout(function(){socket.emit("message",{fetch:"mentions"})},1E4);setTimeout(function(){socket.emit("message",{fetch:"dms-inbox"})},
2E4);setTimeout(function(){socket.emit("message",{fetch:"dms-outbox"})},3E4);setInterval(function(){z_engine_check_ratelimit()},check_ratelimit_interval*1E3);setInterval(function(){z_engine_update_relative_time("time.home")},update_relative_home_interval*1E3);setInterval(function(){z_engine_update_relative_time("time.mentions")},update_relative_mentions_interval*1E3);setInterval(function(){z_engine_update_relative_time("time.dms")},update_relative_dms_interval*1E3);setInterval(function(){$$("time.threaded").length>
0&&z_engine_update_relative_time("time.threaded")},update_relative_home_interval*1E3);setInterval(function(){paused||z_engine_prune_tweets()},prune_tweets_interval*1E3);setInterval(function(){paused||z_engine_stream_queue()},stream_queue_interval*1E3);setInterval(function(){z_engine_geo()},geo_refresh_interval*1E3);setInterval(function(){users_array=$w(store.get("users")).uniq()},5E3)}else if(a.loaded&&loaded)$("new-tweet").enable();else if(a["delete"]){b=typeof a["delete"].status=="object"?a["delete"].status.id_str:
0;z_engine_drop_tweet(b)}else if(a.direct_message){z_engine_tweet(a.direct_message,"dms");a.direct_message.sender.screen_name!=screen_name&&z_engine_notification(a.direct_message.sender.profile_image_url,"@"+a.direct_message.sender.screen_name+" sent a direct message",a.direct_message.text)}else if(a.dms){dms_loaded++;a.dms.each(function(f){content_dms_queued.push(JSON.stringify(f))});switch(dms_loaded){case 1:$("loading-inbox").fade();$("loading-outbox").appear();break;case 2:$("loading-outbox").fade()}}else if(a.event)switch(a.event){case "block":b=
$w(store.get("mention_blocks")).uniq().strip();var c=$w(store.get("user_blocks")).uniq().strip(),d=false,j=false;b.each(function(f){if(f==a.target.screen_name)d=true});c.each(function(f){if(f==a.target.screen_name)j=true});d||store.set("mention_blocks",current_blocks.join(" ")+" "+a.target.screen_name);j||store.set("user_blocks",current_blocks.join(" ")+" "+a.target.screen_name);break;case "unblock":b=$w(store.get("user_blocks")).uniq().strip();c=$w(store.get("user_blocks")).uniq().strip();var h=
"",i="";b.each(function(f){if(f!=a.target.screen_name)h+=f+" "});c.each(function(f){if(f!=a.target.screen_name)i+=f+" "});store.set("mention_blocks",h);store.set("user_blocks",i);break;case "favorite":a.source.screen_name!=screen_name&&z_engine_notification(a.source.profile_image_url,"@"+a.source.screen_name+" faved your tweet!",a.target_object.text);break;case "unfavorite":break;case "follow":a.source.screen_name!=screen_name&&z_engine_notification(a.source.profile_image_url,"@"+a.source.screen_name+
" is following you!",a.source.description);break;case "list_member_added":a.source.screen_name!=screen_name&&z_engine_notification(a.source.profile_image_url,"@"+a.source.screen_name+" put you in "+a.target_object.full_name+"!",a.target_object.description);break;case "list_created":break;case "list_destroyed":break;case "list_member_removed":break;case "list_updated":break;case "list_user_subscribed":a.source.screen_name!=screen_name&&z_engine_notification(a.source.profile_image_url,"@"+a.source.screen_name+
" subscribed to "+a.target_object.full_name+"!",a.target_object.description);break;case "list_user_unsubscribed":break;case "scrub_geo":break;case "user_update":break;default:console.log(JSON.stringify(a.event));console.log(JSON.stringify(a))}else if(a.friends){b=a.friends.join(" ");store.set("friends",b)}else if(a.home){a.home.each(function(f){content_queued.push(JSON.stringify(f))});$("loading-home").fade();$("loading-mentions").appear();socket.emit("message",{fetch:"userstream"})}else if(a.info){screen_name=
a.info.screen_name;user_id=a.info.user_id;store.set("screen_name",screen_name);store.set("user_id",user_id)}else if(a.klout){c=a.klout;b=a.id_str;if(c!="error"){z_engine_set_klout(c.users,b);klout[c.users[0].twitter_id]=JSON.stringify(c.users)}else if($("klout-"+b)){$("klout-"+b).setStyle("cursor: default;");$("klout-"+b).setAttribute("onclick","");$("klout-"+b).addTip("<big><strong>?</strong></big>",kloutinfo_params)}}else if(a.mentions){a.mentions.each(function(f){content_mentions_queued.push(JSON.stringify(f))});
$("loading-mentions").fade();$("loading-inbox").appear()}else if(a.rates){rates=JSON.stringify(a.rates);a.rates.remaining_hits<=10&&z_engine_notification("","notice!","you have "+a.rates.remaining_hits+" (of "+a.rates.hourly_limit+") request tokens left!")}else if(a.retweet_info){c=a.retweet_info;b=c.retweeted_status.id_str;if($("comment-"+b)){if($("rt-"+b)){$("rt-"+b).writeAttribute("src","img/rtd.png");$("rt-"+b).writeAttribute("onclick","z_engine_destroy('"+c.id_str+"','rt');")}if($("rt-"+b+"-mentioned")){$("rt-"+
b+"-mentioned").writeAttribute("src","img/rtd.png");$("rt-"+b+"-mentioned").writeAttribute("onclick","z_engine_destroy('"+c.id_str+"','rt');")}if($("rt-"+b+"-threaded")){$("rt-"+b+"-threaded").writeAttribute("src","img/rtd.png");$("rt-"+b+"-threaded").writeAttribute("onclick","z_engine_destroy('"+c.id_str+"','rt');")}}}else if(a.server_event){console.log(JSON.stringify(a));switch(a.server_event){case "end":$("new-tweet").disable();z_engine_notification("","notice!","lost connection to the userstream, reconnecting...");
break;case "error":$("new-tweet").disable();z_engine_notification("","notice!","error occurred, reconnecting...")}}else if(a.shorten){b=$("new-tweet").getValue().replace(a.original,a.shorten);$("new-tweet").setValue(b)}else if(a.show)content_threads_queued.push(JSON.stringify(a.show));else if(a.text&&a.user&&a.created_at){content_queued.push(JSON.stringify(a));if(paused){$("paused-count").update("("+pttid+")");pttid++}a.retweeted_status&&a.retweeted_status.user.screen_name==screen_name&&a.user.screen_name!=
screen_name&&z_engine_notification(a.user.profile_image_url,"@"+a.user.screen_name+" retweeted you!",a.retweeted_status.text)}}else if(b.isJSON())b.evalJSON(true)?console.log("unknown error, output: "+b):console.log("json could not be properly evaluated, skipping");else console.log("not json, skipping")});socket.on("disconnect",function(){$("new-tweet").disable()})}function z_engine_check_ratelimit(){socket.emit("message",{fetch:"rates"})}
...
function z_engine_tweet_mentioned_string(a){var b="";a.user_mentions.uniq().each(function(c){if(c.screen_name!=screen_name)b+="@"+c.screen_name+" "});return $w(b).uniq().join(" ")}function z_engine_tweet_pause(){if(paused){paused=false;$("paused-count").fade();$("paused-count").update("(0)");$("pause").update("stop");pttid=0}else{paused=true;$("pause").update("start");$("paused-count").appear()}}
