...
else if(a["delete"]){b=typeof a["delete"].status=="object"?a["delete"].status.id_str:0;z_engine_drop_tweet(b)}else if(a.direct_message){z_engine_tweet(a.direct_message,"dms");if(a.direct_message.sender.screen_name!=screen_name){b=a.direct_message.sender.profile_image_url;var d=a.direct_message.text,e="@"+a.direct_message.sender.screen_name+" sent a direct message";z_engine_notification(b,e,d)}}else if(a.dms){dms_loaded++;a.dms.each(function(f){z_engine_tweet(f,"dms")});switch(dms_loaded){case 1:$("loading-inbox").fade();
$("loading-outbox").appear();break;case 2:$("loading-outbox").fade()}rates--}else if(a.event)switch(a.event){case "block":b=$w(store.get("mention_blocks")).uniq().strip();d=$w(store.get("user_blocks")).uniq().strip();var j=false,k=false;b.each(function(f){if(f==a.target.screen_name)j=true});d.each(function(f){if(f==a.target.screen_name)k=true});j||store.set("mention_blocks",current_blocks.join(" ")+" "+a.target.screen_name);k||store.set("user_blocks",current_blocks.join(" ")+" "+a.target.screen_name);
break;case "unblock":b=$w(store.get("user_blocks")).uniq().strip();d=$w(store.get("user_blocks")).uniq().strip();var l="",m="";b.each(function(f){if(f!=a.target.screen_name)l+=f+" "});d.each(function(f){if(f!=a.target.screen_name)m+=f+" "});store.set("mention_blocks",l);store.set("user_blocks",m);break;case "favorite":if(a.source.screen_name!=screen_name){b=a.source.profile_image_url;d=a.target_object.text;e="@"+a.source.screen_name+" faved your tweet!";z_engine_notification(b,e,d)}break;case "unfavorite":break;
case "follow":if(a.source.screen_name!=screen_name){b=a.source.profile_image_url;d=a.source.description;e="@"+a.source.screen_name+" is following you!";z_engine_notification(b,e,d)}break;case "list_member_added":if(a.source.screen_name!=screen_name){b=a.source.profile_image_url;d=a.target_object.description;e="@"+a.source.screen_name+" put you in "+a.target_object.full_name+"!";z_engine_notification(b,e,d)}break;case "list_created":break;case "list_destroyed":break;case "list_member_removed":break;
case "list_updated":break;case "list_user_subscribed":if(a.source.screen_name!=screen_name){b=a.source.profile_image_url;d=a.target_object.description;e="@"+a.source.screen_name+" subscribed to "+a.target_object.full_name+"!";z_engine_notification(b,e,d)}break;case "list_user_unsubscribed":break;case "scrub_geo":break;case "user_update":break;default:console.log(JSON.stringify(a.event));console.log(JSON.stringify(a))}else if(a.friends){b=a.friends.join(" ");store.set("friends",b)}else if(a.home){a.home.each(function(f){z_engine_tweet(f,
"home")});$("loading-home").fade();$("loading-mentions").appear();rates--;socket.emit("message",{fetch:"userstream"})}else if(a.info){screen_name=a.info.screen_name;user_id=a.info.user_id;store.set("screen_name",screen_name);store.set("user_id",user_id)}else if(a.klout){if(store.get("klout")=="on"){d=a.klout;b=a.id_str;if(d!="error"){z_engine_set_klout(d.users,b);klout[d.users[0].twitter_id]=JSON.stringify(d.users)}else if($("klout-"+b)){$("klout-"+b).setStyle("cursor: default;");$("klout-"+b).setAttribute("onclick",
"");$("klout-"+b).addTip("<big><strong>?</strong></big>",kloutinfo_params)}}}else if(a.mentions){a.mentions.each(function(f){z_engine_tweet(f,"mentions")});$("loading-mentions").fade();$("loading-inbox").appear();rates--}else if(a.rates){rates=a.rates.remaining_hits;a.rates.remaining_hits<=10&&z_engine_notification("","notice!","you have "+a.rates.remaining_hits+" (of "+a.rates.hourly_limit+") request tokens left!")}else if(a.retweet_info){d=a.retweet_info;b=d.retweeted_status.id_str;e=d.id_str;content_rts_stored[b]=
JSON.stringify(d);if($("rt-"+b)){$("rt-"+b).writeAttribute("src","img/rtd.png");$("rt-"+b).writeAttribute("onclick","z_engine_destroy('"+e+"','rt');")}if($("rt-"+b+"-mentioned")){$("rt-"+b+"-mentioned").writeAttribute("src","img/rtd.png");$("rt-"+b+"-mentioned").writeAttribute("onclick","z_engine_destroy('"+e+"','rt');")}if($("rt-"+b+"-threaded")){$("rt-"+b+"-threaded").writeAttribute("src","img/rtd.png");$("rt-"+b+"-threaded").writeAttribute("onclick","z_engine_destroy('"+e+"','rt');")}}else if(a.server_event){console.log(JSON.stringify(a));
switch(a.server_event){case "end":$("new-tweet").disable();z_engine_notification("","notice!","lost connection to the userstream, reconnecting...");break;case "error":$("new-tweet").disable();z_engine_notification("","notice!","error occurred, reconnecting...")}}else if(a.shorten){b=$("new-tweet").getValue().replace(a.original,a.shorten);$("new-tweet").setValue(b)}else if(a.show){d=a.show;b=d.retweeted_status?d.retweeted_status.id_str:d.id_str;content_threads_stored[b]=JSON.stringify(d);z_engine_tweet(d,
"threaded");rates--}else if(a.text&&a.user&&a.created_at){content_queued.push(JSON.stringify(a));if(paused){$("paused-count").update("("+pttid+")");pttid++}if(a.retweeted_status&&a.retweeted_status.user.screen_name==screen_name&&a.user.screen_name!=screen_name){b=a.user.profile_image_url;e=a.retweeted_status.retweet_count-1;d=a.retweeted_status.text;e=e>1?"@"+a.user.screen_name+" and "+e+" others rt'd you!":e==1?"@"+a.user.screen_name+" and 1 other rt'd you!":"@"+a.user.screen_name+" retweeted you!";
z_engine_notification(b,e,d)}}}else if(b.isJSON())b.evalJSON(true)?console.log("unknown error, output: "+b):console.log("json could not be properly evaluated, skipping");else console.log("not json, skipping")});socket.on("disconnect",function(){$("new-tweet").disable()})}else $("new-tweet").setValue("sorry, your browser may not support this client!")}function z_engine_check_ratelimit(){socket.emit("message",{fetch:"rates"})}
...
function z_engine_settings_checked_clicker(a,b){new Event.observe(a,"change",function(c){Event.stop(c);c=$(a).getValue();switch(b){case "lang":store.set(b,c);z_engine_set_language();break;case "stream_interval":store.set(b,c);stream_queue_interval=clearInterval(stream_queue_interval);stream_queue_interval=window.setInterval(function(){paused||this.z_engine_stream_queue()},store.get(b)*1E3);break;default:c=store.get(b)=="on"?"off":"on";store.set(b,c)}})}
...
o=new Element("div",{"class":"right"});if(y){i=n>0?"retweeted by "+d+" and "+n+" others!":"retweeted by "+d+"!";n=new Element("span",{"class":"rtd"});n.update(" ");i=new Element("img",{src:"img/rtd2.png",alt:"",title:i});j=new Element("a",{target:"_blank",href:"http://twitter.com/"+d});j.update(d);n.insert({top:i});n.insert({bottom:j});o.insert({bottom:n})}if(b!="dms"&&r&&r.full_name){n=new Element("span",{"class":"place",id:"place-"+g});r=new Element("a",{target:"_blank",href:"http://maps.google.com?q="+
r.full_name});i=new Element("img",{src:"img/plc.png",alt:""});r.update(i);n.update(r);o.insert({bottom:n})}B.insert({bottom:o});D.insert({bottom:B});n=new Element("div",{"class":"clearer"});n.update("&nbsp;");D.insert({bottom:n});B=new Element("div",{"class":"comment-text"});o=new Element("p",{id:"comment-"+g+"-text"});r=new Element("div",{id:"right-"+g,"class":"right"});o.insert(E);o.insert({bottom:r});E=new Element("div",{id:"text-"+g});E.update(z_engine_parse_tweet(w));o.insert({bottom:E});B.insert(o);
D.insert({bottom:B});C.insert(D);f.insert(z);f.insert({bottom:C});f.insert({bottom:n});new Element.extend(h);new Element.extend(p);new Element.extend(r);new Element.extend(f);switch(b){case "dms":if(c!=screen_name)$("dms-inbox-timeline").insert({top:f});else c==screen_name&&$("dms-outbox-timeline").insert({top:f});break;case "home":$("home-timeline").insert({top:f});break;case "mentions":$("mentions-timeline").insert({bottom:f})}if(q&&b!="mentions"&&b!="threaded"&&c!=screen_name&&!a.retweeted_status){q=
