...
a);klout[b.users[0].twitter_id]=JSON.stringify(b.users)}else if($("klout-"+a)){$("klout-"+a).setStyle("cursor: default;");$("klout-"+a).writeAttribute("onclick","");$("klout-"+a).addTip("<big><strong>?</strong></big>",kloutinfo_params)}}});socket.on("loaded",function(){loaded=true;$("new-tweet").setValue("");$("new-tweet").enable();z_engine_fetch_timeline("home");z_engine_check_ratelimit();z_engine_fetch_timeline("home");z_engine_fetch_timeline.delay(10,"mentions");z_engine_fetch_timeline.delay(20,
"dms-inbox");z_engine_fetch_timeline.delay(30,"dms-outbox");new PeriodicalExecuter(function(){z_engine_update_relative_time("time.home")},update_relative_home_interval);new PeriodicalExecuter(function(){z_engine_update_relative_time("time.mentions")},update_relative_mentions_interval);new PeriodicalExecuter(function(){z_engine_update_relative_time("time.dms")},update_relative_dms_interval);new PeriodicalExecuter(function(){z_engine_update_relative_time("time.threaded")},update_relative_home_interval);
new PeriodicalExecuter(function(){z_engine_check_ratelimit()},check_ratelimit_interval);new PeriodicalExecuter(function(){paused||z_engine_prune_tweets()},prune_tweets_interval);stream_queue_interval=new PeriodicalExecuter(function(){paused||z_engine_stream_queue()},store.get("stream_interval"));store.get("geo")=="on"&&new PeriodicalExecuter(function(){z_engine_geo()},geo_refresh_interval);new PeriodicalExecuter(function(){z_engine_input()},1);var a=$w(store.get("users").strip()).uniq(),b="";a.each(function(c){b+=
c.replace(/@/i,"")+" "});new Autocompleter.Local("new-tweet","autocompleter",a,{choices:20,minChars:2,tokens:" ",afterUpdateElement:function(){$("new-tweet").setValue($("new-tweet").getValue()+" ")}});new Autocompleter.Local("new-dm-user","autocompleter-dm",$w(b.strip()).uniq(),{choices:20,minChars:2});store.get("geo")=="on"&&new HotKey("l",function(){z_engine_geo()},{shiftKey:true});new HotKey("o",function(){z_engine_clear_timeline()},{shiftKey:true});new HotKey("p",function(){z_engine_tweet_pause()},
{shiftKey:true});new HotKey("s",function(){z_engine_shorten_urls()},{shiftKey:true});progress_bar=new Control.ProgressBar("progress-bar");new Control.Modal($(document.body).down("[href=#settings-div]"),{className:"window",closeOnClick:"overlay",fade:true,overlayOpacity:0.5});tabs=new Control.Tabs("tabbed",{beforechange:function(){previous_tab=z_engine_current_timeline()},afterChange:function(){var c=z_engine_current_timeline();switch(c){case "dms-inbox-timeline":case "dms-outbox-timeline":$("new-dm-user").appear();
break;default:$("new-tweet").removeClassName("dm");$("new-dm-user").fade().setValue("")}if(c!="threaded-timeline"){$("threaded-timeline").update();latest_threaded_id=0}}});tabs.first();new Event.observe("new-tweet","keyup",function(){z_engine_input()});new Event.observe("new-tweet","keydown",function(){z_engine_input()});new Event.observe("new-tweet-form","submit",function(c){Event.stop(c);z_engine_send_tweet()});new Event.observe("pause","click",function(c){Event.stop(c);z_engine_tweet_pause()});
z_engine_image_dropper();z_engine_notification_setup();z_engine_settings_setup()});socket.on("mentions",function(a){a.each(function(b){z_engine_tweet(b,"mentions bottom")});$("loading-mentions").fade();$("loading-inbox").appear();rates--});socket.on("rates",function(a){rates=a.remaining_hits;a.remaining_hits<=10&&z_engine_notification("","notice!","you have "+a.remaining_hits+" (of "+a.hourly_limit+") request tokens left!")});socket.on("related",function(a){try{a.data.each(function(c){if(c.kind==
"Tweet"){switch(c.annotations.ConversationRole){case "Ancestor":var g="bottom";break;case "Descendant":g="top";break;case "Fork":g="top"}z_engine_tweet(c.value,"threaded "+g)}else c.results.each(function(f){if(f.kind=="Tweet"){switch(f.annotations.ConversationRole){case "Ancestor":var e="bottom";break;case "Descendant":e="top";break;case "Fork":e="top"}z_engine_tweet(f.value,"threaded "+e)}})});content_stored[a.origin]&&z_engine_tweet(content_stored[a.origin].evalJSON(true),"threaded top")}catch(b){console.log(b)}rates--});
socket.on("retweet_info",function(a){var b=a.retweeted_status.id_str,c=a.id_str;content_rts_stored[b]=JSON.stringify(a);if($("rt-"+b)){$("rt-"+b).writeAttribute("title","undo retweet");$("rt-"+b).writeAttribute("src","img/rtd.png");$("rt-"+b).writeAttribute("onclick","z_engine_destroy('"+c+"','rt');")}if($("rt-"+b+"-mentioned")){$("rt-"+b+"-mentioned").writeAttribute("title","undo retweet");$("rt-"+b+"-mentioned").writeAttribute("src","img/rtd.png");$("rt-"+b+"-mentioned").writeAttribute("onclick",
"z_engine_destroy('"+c+"','rt');")}if($("rt-"+b+"-threaded")){$("rt-"+b+"-threaded").writeAttribute("title","undo retweet");$("rt-"+b+"-threaded").writeAttribute("src","img/rtd.png");$("rt-"+b+"-threaded").writeAttribute("onclick","z_engine_destroy('"+c+"','rt');")}});socket.on("server_error",function(a){switch(a.event){case "end":$("new-tweet").disable();z_engine_notification("","notice!","lost connection to the userstream, reconnecting...");break;case "error":$("new-tweet").disable();z_engine_notification("",
"notice!","error occurred, reconnecting...")}});socket.on("shorten",function(a){a=$("new-tweet").getValue().replace(a.original,a.shorten);$("new-tweet").setValue(a)});socket.on("show",function(a){content_stored[a.retweeted_status?a.retweeted_status.id_str:a.id_str]=JSON.stringify(a);z_engine_tweet(a,"threaded bottom");rates--});socket.on("tweet",function(a){content_queued.push(JSON.stringify(a));if(paused){$("paused-count").update("("+pttid+")");pttid++}if(a.retweeted_status&&a.retweeted_status.user.screen_name==
screen_name&&a.user.screen_name!=screen_name){var b=a.retweeted_status.retweet_count-1;z_engine_notification(a.user.profile_image_url,b>1?"@"+a.user.screen_name+" and "+b+" others rt'd you!":b==1?"@"+a.user.screen_name+" and 1 other rt'd you!":"@"+a.user.screen_name+" retweeted you!",a.retweeted_status.text)}});socket.on("disconnect",function(){$("new-tweet").disable()})}else $("new-tweet").setValue("sorry, your browser may not support this client!")}
function z_engine_check_ratelimit(){z_engine_fetch_timeline("rates")}function z_engine_clear_timeline(){var a=z_engine_current_timeline();if(z_engine_css3()){$(a).addClassName("comment-parent-drop").removeClassName("comment-parent");$(a).update().appear().delay(0.5)}else new Effect.Fade(a,{duration:0.5,afterFinish:function(){$(a).update().appear()}})}
...
function z_engine_settings_get(a,b){switch(b){case "notifications_timeout":$(a).setValue(store.get(b));break;case "stream_interval":$(a).setValue(store.get(b));break;case "lang":var c=store.get("lang");$(c).writeAttribute("selected","selected");break;default:store.get(b)=="on"&&$(a).click()}}
...
function z_engine_tweet(a,b){var c=$w(b),g=c[0];c=c[1];if(g!="dms")if(a.retweeted_status){f=a.retweeted_status.user.screen_name;e=a.user.screen_name;j=a.retweeted_status.user.profile_image_url;d=(new Date(a.retweeted_status.created_at)).toLocaleString().replace(/GMT.+/,"");i=a.retweeted_status.user.description;m=a.retweeted_status.entities;h=a.retweeted_status.favorited;l=a.retweeted_status.user.followers_count;p=a.retweeted_status.user.friends_count;k=a.retweeted_status.id_str;w=a.retweeted_status.user.listed_count;
n=a.retweeted_status.user.location;y=a.retweeted_status.user["protected"];t=a.retweeted_status.place;o=a.retweeted_status.in_reply_to_screen_name;D=a.retweeted_status.in_reply_to_status_id_str;q=a.retweeted_status.retweet_count-1;E=true;J=a.retweeted_status.source.stripTags();A=a.retweeted_status.text;z=a.retweeted_status.user.statuses_count;F=a.retweeted_status.user.id;r=a.retweeted_status.user.verified}else var f=a.user.screen_name,e=false,j=a.user.profile_image_url,d=(new Date(a.created_at)).toLocaleString().replace(/GMT.+/,
""),i=a.user.description,m=a.entities,h=a.favorited,l=a.user.followers_count,p=a.user.friends_count,k=a.id_str,w=a.user.listed_count,n=a.user.location,y=a.user["protected"],t=a.place,o=a.in_reply_to_screen_name,D=a.in_reply_to_status_id_str,q=a.retweet_count-1,E=false,J=a.source.stripTags(),A=a.text,z=a.user.statuses_count,F=a.user.id,r=a.user.verified;else{f=a.sender.screen_name;j=a.sender.profile_image_url;d=(new Date(a.created_at)).toLocaleString().replace(/GMT.+/,"");i=a.sender.description;m=
false;l=a.sender.followers_count;p=a.sender.friends_count;k=a.id_str;w=a.sender.listed_count;n=a.sender.location;y=false;o=a.recipient.screen_name;q=0;E=false;A=a.text;z=a.sender.statuses_count;F=a.sender_id;r=a.sender.verified}var M=false,N=false,O=false,P=false,u=false;if(g!="threaded"&&content_stored[k])u=true;g!="dms"&&$w(store.get("client_blocks")).uniq().each(function(B){if(B.replace("'","")==J)M=true});if(m){typeof m.hashtags=="object"&&$w(store.get("hashtag_blocks")).uniq().each(function(B){m.hashtags.uniq().each(function(K){if(B==
K)N=true})});typeof m.user_mentions=="object"&&$w(store.get("mention_blocks")).uniq().each(function(B){m.user_mentions.uniq().each(function(K){if(B==K)O=true})})}$w(store.get("user_blocks")).uniq().each(function(B){if(B==f)P=true});if(!u&&!M&&!N&&!O&&!P){(new Image).src=j;content_stored[k]=JSON.stringify(a);z_engine_remember_author(f);u="";if(i!=null&&i!="")u+=i+"<br /><br />";if(n!=null&&n!="")u+="location: <strong>"+n+"</strong><br />";u+="tweets: <strong>"+z+"</strong><br />";u+="following: <strong>"+
p+"</strong><br />";u+="followers: <strong>"+l+"</strong><br />";u+="listed: <strong>"+w+"</strong>";if(m){w=z_engine_tweet_mentioned(m);i=z_engine_tweet_mentioned_string(m)}else i=w=false;z=new Element("br");l=g!="dms"?new Element("div",{ondblclick:'z_engine_reply("'+f+'", "'+k+'", "'+i+'");',id:"comment-"+k,"class":"comment-parent"}):new Element("div",{ondblclick:'z_engine_reply_dm("'+F+'", "'+f+'");',id:"comment-"+k,"class":"comment-parent"});n=new Element("div",{"class":"comment-profile-wrapper left"});
var H=new Element("div",{"class":"comment-gravatar"});p=new Element("img",{id:"av-"+k,src:j,style:"height: 50px; width: 50px; cursor: pointer;",alt:""});H.insert(p);n.insert(H);H=new Element("div",{id:"comment-"+k+"content","class":"comment-content-wrapper right"});var I=f!=screen_name?w?new Element("div",{"class":"comment-body-mentioned"}):new Element("div",{"class":"comment-body"}):new Element("div",{"class":"comment-body-me"}),s=new Element("div",{"class":"comment-arrow"});I.insert(s);var G=new Element("div",
{"class":"post-date"});s=new Element("div",{"class":"left",id:"left-"+k});var Q=new Element("a",{target:"_blank",href:"http://twitter.com/"+f});Q.update("@"+f+" ");var R=new Element("span");R.update(translation.wrote_this);if(g!="dms")var v=new Element("a",{target:"_blank",id:"comment-"+k+"-relative-date",href:"http://twitter.com/"+f+"/status/"+k});var L=new Element("time",{datetime:d,"class":g});L.update(relative_time(d));g!="dms"&&v.insert(L);s.insert(Q);s.insert({bottom:R});if(g!="dms"){s.insert({bottom:v});
if(D){v=new Element("span");v.update(translation.in_reply_to);d=g=="threaded"?new Element("a",{target:"_blank",href:"http://twitter.com/"+o+"/status/"+D}):new Element("span",{onclick:'z_engine_threaded("'+k+'","'+D+'");',style:"cursor: pointer;"});d.update("@"+o+" ");s.insert(v);s.insert({bottom:d})}o=new Element("span",{"class":"via"});o.update(translation.via+" "+J);s.insert({bottom:o})}else{s.insert({bottom:L});if(o){v=new Element("span");v.update(" in reply to ");d=new Element("a",{target:"_blank",
href:"http://twitter.com/"+o});d.update(o+" ");s.insert(v);s.insert({bottom:d})}}if(r){r=new Element("span");r.update(" ");o=new Element("img",{src:"img/ver.png",alt:"",title:"verified user"});r.insert({bottom:o});s.insert({bottom:r})}G.insert(s);r=new Element("div",{"class":"right"});if(E){o=q>0?"retweeted by "+e+" and "+q+" others!":"retweeted by "+e+"!";q=new Element("span",{"class":"rtd"});q.update(" ");o=new Element("img",{src:"img/rtd2.png",alt:"",title:o});v=new Element("a",{target:"_blank",
href:"http://twitter.com/"+e});v.update(e);q.insert({top:o});q.insert({bottom:v});r.insert({bottom:q})}if(g!="dms"&&t&&t.full_name){q=new Element("span",{"class":"place",id:"place-"+k});o=new Element("a",{target:"_blank",href:"http://maps.google.com?q="+t.full_name});t=new Element("img",{src:"img/plc.png",alt:"",title:t.full_name});o.update(t);q.update(o);r.insert({bottom:q})}G.insert({bottom:r});I.insert({bottom:G});q=new Element("div",{"class":"clearer"});q.update("&nbsp;");I.insert({bottom:q});
G=new Element("div",{"class":"comment-text"});r=new Element("p",{id:"comment-"+k+"-text"});t=new Element("div",{id:"right-"+k,"class":"right"});r.insert(z);r.insert({bottom:t});z=new Element("div",{id:"text-"+k,"class":"comment-text-select"});z.update(z_engine_parse_tweet(A));r.insert({bottom:z});G.insert(r);I.insert({bottom:G});H.insert(I);l.insert(n);l.insert({bottom:H});l.insert({bottom:q});new Element.extend(p);new Element.extend(s);new Element.extend(t);new Element.extend(l);switch(c){case "bottom":var x=
{bottom:l};break;case "top":x={top:l}}switch(g){case "dms":if(f!=screen_name)$("dms-inbox-timeline").insert(x);else f==screen_name&&$("dms-outbox-timeline").insert(x);break;case "home":$("home-timeline").insert(x);break;case "mentions":$("mentions-timeline").insert(x)}if(w&&g!="mentions"&&g!="threaded"&&f!=screen_name&&!a.retweeted_status){x=cloneNodeWithEvents(l);x.writeAttribute("id","comment-"+k+"-mentioned");s.writeAttribute("id","left-"+k+"-mentioned");t.writeAttribute("id","right-"+k+"-mentioned");
p.writeAttribute("id","av-"+k+"-mentioned");new Element.extend(x);switch(c){case "bottom":var C={bottom:x};break;case "top":C={top:x}}$("mentions-timeline").insert(C);z_engine_tweet_buttons("mentions",k,f,e,F,A,y,h,E,i,u);z_engine_fade_down("comment-"+k+"-mentioned");z_engine_notification(j,"@"+f+" mentioned you!",A)}if(g=="threaded"){s.writeAttribute("id","left-"+k+"-threaded");t.writeAttribute("id","right-"+k+"-threaded");p.writeAttribute("id","av-"+k+"-threaded");C=cloneNodeWithEvents(l);C.writeAttribute("id",
"comment-"+k+"-threaded");new Element.extend(C);switch(c){case "bottom":var S={bottom:C};break;case "top":S={top:C}}$("threaded-timeline").insert(S);z_engine_tweet_buttons("threaded",k,f,e,F,A,y,h,E,i,u);z_engine_fade_down("comment-"+k+"-threaded")}if($("comment-"+k)){z_engine_tweet_buttons(g,k,f,e,F,A,y,h,E,i,u);z_engine_fade_down("comment-"+k)}}g=="threaded"&&D&&z_engine_threaded(false,D)}
