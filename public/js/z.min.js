...
if(store.get("sound")=="on"&&store.get("sound_src").length>0){var audio=new Audio;audio.src=store.get("sound_src")}store.get("stream_interval")||store.set("stream_interval",1.5);var threaded_cutoff=50,translation="",update_relative_dms_interval=60,update_relative_home_interval=15,update_relative_mentions_interval=30,user_id=0,userinfo_params={className:"user",hideDelay:3,hideOn:"mouseout",showOn:"click",offset:[11,0],stem:true,target:true,targetJoint:["right","middle"],tipJoint:["left","middle"]};
...
check_ratelimit_interval);new PeriodicalExecuter(function(){paused||z_engine_prune_tweets()},prune_tweets_interval);stream_queue_interval=new PeriodicalExecuter(function(){paused||z_engine_stream_queue()},store.get("stream_interval"));store.get("geo")=="on"&&new PeriodicalExecuter(function(){z_engine_geo()},geo_refresh_interval);new PeriodicalExecuter(function(){z_engine_input()},1);(new Control.Tabs("tabbed",{afterChange:function(){var f=z_engine_current_timeline();switch(f){case "dms-inbox-timeline":case "dms-outbox-timeline":$("new-dm-user").appear();
break;default:$("new-dm-user").fade().setValue("")}if(f!="threaded-timeline"){$("threaded-timeline").update();latest_threaded_id=0}}})).first();b=$w(store.get("users").strip()).uniq();var c="";b.each(function(f){c+=f.replace(/@/i,"")+" "});new Autocompleter.Local("new-tweet","autocompleter",b,{choices:20,minChars:2,tokens:" ",afterUpdateElement:function(){$("new-tweet").setValue($("new-tweet").getValue()+" ")}});new Autocompleter.Local("new-dm-user","autocompleter-dm",$w(c.strip()).uniq(),{choices:20,
minChars:2});store.get("geo")=="on"&&new HotKey("l",function(){z_engine_geo()},{shiftKey:true});new HotKey("o",function(){z_engine_clear_timeline()},{shiftKey:true});new HotKey("p",function(){z_engine_tweet_pause()},{shiftKey:true});new HotKey("s",function(){z_engine_shorten_urls()},{shiftKey:true});b=(new Element("button",{className:"window-close"})).update("X");(new Control.Modal($(document.body).down("[href=/account/settings]"),{className:"window",closeOnClick:b,fade:true,method:"GET",overlayOpacity:0.5})).container.insert({top:b});
...
"",m="";b.each(function(f){if(f!=a.target.screen_name)l+=f+" "});d.each(function(f){if(f!=a.target.screen_name)m+=f+" "});store.set("mention_blocks",l);store.set("user_blocks",m);break;case "favorite":if(a.source.screen_name!=screen_name){b=a.source.profile_image_url;d=a.target_object.text;e="@"+a.source.screen_name+" faved your tweet!";z_engine_notification(b,e,d)}break;case "unfavorite":break;case "follow":if(a.source.screen_name!=screen_name){b=a.source.profile_image_url;d=a.source.description;
...
function z_engine_favorite(a){socket.emit("message",{favorite:{status:{id_str:a}}})}function z_engine_fetch_timeline(a){socket.emit("message",{fetch:a})}function z_engine_geo(){navigator.geolocation&&navigator.geolocation.getCurrentPosition(z_engine_geo_set,z_engine_geo_error,{enableHighAccuracy:geo_high_accuracy,maximumAge:0,timeout:geo_timeout*1E3})}function z_engine_geo_error(a){switch(a.code){case 0:case 1:case 2:case 3:longitude=latitude=false}}
function z_engine_geo_set(a){latitude=a.coords.latitude;longitude=a.coords.longitude}function z_engine_get_height(){var a=document;return Math.max(Math.max(a.body.scrollHeight,a.documentElement.scrollHeight),Math.max(a.body.offsetHeight,a.documentElement.offsetHeight),Math.max(a.body.clientHeight,a.documentElement.clientHeight))}function z_engine_get_klout(a,b,c){if(klout[b]){a=klout[b].evalJSON(true);z_engine_set_klout(a,c)}else socket.emit("message",{klout:a,id_str:c})}
...
function z_engine_tweet(a,b){if(b!="dms")if(a.retweeted_status){c=a.retweeted_status.user.screen_name;d=a.user.screen_name;e=a.retweeted_status.user.profile_image_url;i=(new Date(a.retweeted_status.created_at)).toLocaleString().replace(/GMT.+/,"");k=a.retweeted_status.user.description;l=a.retweeted_status.entities;m=a.retweeted_status.favorited;f=a.retweeted_status.user.followers_count;h=a.retweeted_status.user.friends_count;g=a.retweeted_status.id_str;q=a.retweeted_status.user.location;t=a.retweeted_status.user["protected"];
r=a.retweeted_status.place;j=a.retweeted_status.in_reply_to_screen_name;u=a.retweeted_status.in_reply_to_status_id_str;n=a.retweeted_status.retweet_count-1;y=true;F=a.retweeted_status.source.stripTags();w=a.retweeted_status.text;z=a.retweeted_status.user.statuses_count;A=a.retweeted_status.user.id;o=a.retweeted_status.user.verified}else var c=a.user.screen_name,d=false,e=a.user.profile_image_url,i=(new Date(a.created_at)).toLocaleString().replace(/GMT.+/,""),k=a.user.description,l=a.entities,m=a.favorited,
f=a.user.followers_count,h=a.user.friends_count,g=a.id_str,q=a.user.location,t=a.user["protected"],r=a.place,j=a.in_reply_to_screen_name,u=a.in_reply_to_status_id_str,n=a.retweet_count-1,y=false,F=a.source.stripTags(),w=a.text,z=a.user.statuses_count,A=a.user.id,o=a.user.verified;else{c=a.sender.screen_name;e=a.sender.profile_image_url;i=(new Date(a.created_at)).toLocaleString().replace(/GMT.+/,"");k=a.sender.description;l=false;f=a.sender.followers_count;h=a.sender.friends_count;g=a.id_str;q=a.sender.location;
t=false;j=a.recipient.screen_name;n=0;y=false;w=a.text;z=a.sender.statuses_count;A=a.sender_id;o=a.sender.verified}var I=false,J=false,K=false,L=false,s=false;if(b!="threaded"&&content_stored[g])s=true;b!="dms"&&$w(store.get("client_blocks")).uniq().each(function(x){if(x.replace("'","")==F)I=true});if(l){typeof l.hashtags=="object"&&$w(store.get("hashtag_blocks")).uniq().each(function(x){l.hashtags.uniq().each(function(G){if(x==G)J=true})});typeof l.user_mentions=="object"&&$w(store.get("mention_blocks")).uniq().each(function(x){l.user_mentions.uniq().each(function(G){if(x==
...
{target:"_blank",id:"comment-"+g+"-relative-date",href:"http://twitter.com/"+c+"/status/"+g});var H=new Element("time",{datetime:i,"class":b});H.update(relative_time(i));b!="dms"&&v.insert(H);p.insert(M);p.insert({bottom:N});if(b!="dms"){p.insert({bottom:v});if(u){i=new Element("span");i.update(translation.in_reply_to);v=b=="threaded"?new Element("a",{target:"_blank",href:"http://twitter.com/"+j+"/status/"+u}):new Element("span",{onclick:'z_engine_threaded("'+g+'","'+u+'");',style:"cursor: pointer;"});
v.update("@"+j+" ");p.insert(i);p.insert({bottom:v})}j=new Element("span",{"class":"via"});j.update(translation.via+" "+F);p.insert({bottom:j})}else{p.insert({bottom:H});if(j){i=new Element("span");i.update(" in reply to ");v=new Element("a",{target:"_blank",href:"http://twitter.com/"+j});v.update(j+" ");p.insert(i);p.insert({bottom:v})}}if(o){o=new Element("span");o.update(" ");j=new Element("img",{src:"img/ver.png",alt:"",title:"verified user"});o.insert({bottom:j});p.insert({bottom:o})}B.insert(p);
o=new Element("div",{"class":"right"});if(y){j=n>0?"retweeted by "+d+" and "+n+" others!":"retweeted by "+d+"!";n=new Element("span",{"class":"rtd"});n.update(" ");j=new Element("img",{src:"img/rtd2.png",alt:"",title:j});i=new Element("a",{target:"_blank",href:"http://twitter.com/"+d});i.update(d);n.insert({top:j});n.insert({bottom:i});o.insert({bottom:n})}if(b!="dms"&&r&&r.full_name){n=new Element("span",{"class":"place",id:"place-"+g});j=new Element("a",{target:"_blank",href:"http://maps.google.com?q="+
r.full_name});r=new Element("img",{src:"img/plc.png",alt:"",title:r.full_name});j.update(r);n.update(j);o.insert({bottom:n})}B.insert({bottom:o});D.insert({bottom:B});n=new Element("div",{"class":"clearer"});n.update("&nbsp;");D.insert({bottom:n});B=new Element("div",{"class":"comment-text"});o=new Element("p",{id:"comment-"+g+"-text"});r=new Element("div",{id:"right-"+g,"class":"right"});o.insert(E);o.insert({bottom:r});E=new Element("div",{id:"text-"+g});E.update(z_engine_parse_tweet(w));o.insert({bottom:E});
...
b+'", "tweet");',title:"delete",src:"/img/del.png",id:"del-"+b,alt:""});new Element.extend(h);$("right-"+b).update();$("right-"+b).insert({bottom:h})}z_engine_tweet_right_click(b,"comment-"+b,c,d,e,f,i,l,m,k,a);break;case "mentions":$("av-"+b)&&$("av-"+b).addTip(z_engine_parse_tweet(h),userinfo_params);$("av-"+b+"-mentioned")&&$("av-"+b+"-mentioned").addTip(z_engine_parse_tweet(h),userinfo_params);$("klout-"+b)&&$("klout-"+b).remove();if(c!=screen_name){h=new Element("img",{src:"/img/rep.png",onclick:'z_engine_reply("'+
c+'", "'+b+'-mentioned", "'+f+'");',title:"reply",id:"reply-"+b+"-mentioned",alt:""});new Element.extend(h);if(k)g=new Element("img",{src:"/img/lock.png",style:"cursor: default;",alt:""});else{g=new Element("img",{src:"/img/rt.png",onclick:'z_engine_retweet("'+b+'");',title:"retweet",id:"rt-"+b+"-mentioned",alt:""});new Element.extend(g)}if($("right-"+b)){$("right-"+b).update();$("right-"+b).insert(h);$("right-"+b).insert({bottom:g})}if($("right-"+b+"-mentioned")){$("right-"+b+"-mentioned").update();
$("right-"+b+"-mentioned").insert(h);$("right-"+b+"-mentioned").insert({bottom:g})}}else{h=new Element("img",{onclick:'z_engine_destroy("'+b+'", "tweet");',title:"delete",src:"/img/del.png",id:"del-"+b+"-mentioned",alt:""});new Element.extend(h);if($("right-"+b)){$("right-"+b).update();$("right-"+b).insert({bottom:h})}if($("right-"+b+"-mentioned")){$("right-"+b+"-mentioned").update();$("right-"+b+"-mentioned").insert({bottom:h})}}$("comment-"+b)&&z_engine_tweet_right_click(b,"comment-"+b,c,d,e,f,
i,l,m,k,a);$("comment-"+b+"-mentioned")&&z_engine_tweet_right_click(b,"comment-"+b+"-mentioned",c,d,e,f,i,l,m,k,a);break;case "threaded":$("av-"+b+"-threaded")&&$("av-"+b+"-threaded").addTip(z_engine_parse_tweet(h),userinfo_params);$("klout-"+b)&&$("klout-"+b).remove();if(c!=screen_name){h=new Element("img",{src:"/img/rep.png",onclick:'z_engine_reply("'+c+'", "'+b+'", "'+f+'");',title:"reply",id:"reply-"+b+"-threaded",alt:""});new Element.extend(h);if(k)g=new Element("img",{src:"/img/lock.png",style:"cursor: default;",
alt:""});else{g=new Element("img",{src:"/img/rt.png",onclick:'z_engine_retweet("'+b+'");',id:"rt-"+b+"-threaded",title:"retweet",alt:""});new Element.extend(g)}if($("right-"+b+"-threaded")){$("right-"+b+"-threaded").update();$("right-"+b+"-threaded").insert(h);$("right-"+b+"-threaded").insert({bottom:g})}}else{h=new Element("img",{onclick:'z_engine_destroy("'+b+'", "tweet");',src:"/img/del.png",title:"delete",id:"del-"+b+"-threaded",alt:""});new Element.extend(h);if($("right-"+b+"-threaded")){$("right-"+
b+"-threaded").update();$("right-"+b+"-threaded").insert({bottom:h})}}z_engine_tweet_right_click(b,"comment-"+b+"-threaded",c,d,e,f,i,l,m,k,a)}}function z_engine_tweet_mentioned(a){var b=false;a.user_mentions.uniq().each(function(c){if(c.screen_name==screen_name)b=true});return b}
