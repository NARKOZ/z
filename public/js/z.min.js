...
140)$("new-tweet").setStyle("color: #4d4d4d;");else $("new-tweet").getValue().length>=141&&$("new-tweet").setStyle("color: red;")})}socket.on("connect",function(){loaded||$("new-tweet").setValue("connected...")});socket.on("message",function(b){var d=JSON.stringify(b);if(d.isJSON()&&d.evalJSON(true)){d="";if(b.loaded&&!loaded){loaded=true;$("new-tweet").setValue("");$("new-tweet").enable();new Event.observe("new-tweet-form","submit",function(h){Event.stop(h);z_engine_send_tweet()});z_engine_clicker("home-timeline-click",
...
3E4);setInterval(function(){z_engine_update_relative_time("time.home")},update_relative_home_interval);setInterval(function(){z_engine_update_relative_time("time.mentions")},update_relative_mentions_interval);setInterval(function(){z_engine_update_relative_time("time.dms")},update_relative_dms_interval);setInterval(function(){paused||z_engine_prune_tweets()},prune_tweets_interval);setInterval(function(){paused||z_engine_stream_queue()},stream_queue_interval)}else if(b.loaded&&loaded)$("new-tweet").enable();
else if(b["delete"]){var e=typeof b["delete"].status=="object"?b["delete"].status.id_str:0;z_engine_drop_tweet(e)}else if(b.direct_message){z_engine_tweet(b.direct_message,"dms");b.direct_message.sender.screen_name!=screen_name&&z_engine_notification(b.direct_message.sender.profile_image_url,"@"+b.direct_message.sender.screen_name+" sent a direct message",b.direct_message.text)}else if(b.dms){dms_loaded++;b.dms.each(function(h){z_engine_tweet(h,"dms")});switch(dms_loaded){case 1:$("loading-inbox").fade();
...
"img/rtd.png");$("rt-"+d.retweeted_status.id_str+"-mentioned").writeAttribute("onclick","z_engine_destroy('"+d.retweeted_status.id_str+"','rt');")}}}else if(b.server_event){console.log(JSON.stringify(b));switch(b.server_event){case "end":$("new-tweet").disable();z_engine_notification("","notice!","lost connection to the userstream, refreshing page in 5...");setTimeout(function(){window.location="/"},5E3);break;case "error":$("new-tweet").disable();z_engine_notification("","notice!","userstream error occurred, refreshing page in 5...");
setTimeout(function(){window.location="/"},5E3)}}else if(b.shorten){console.log(JSON.stringify(b.shorten));i=$("new-tweet").getValue().replace(b.original,b.shorten);$("new-tweet").setValue(i)}else if(b.text&&b.user&&b.created_at){content_queued.push(JSON.stringify(b));if(paused){$("paused-count").update("("+pttid+")");pttid++}b.retweeted_status&&b.retweeted_status.user.screen_name==screen_name&&b.user.screen_name!=screen_name&&z_engine_notification(b.user.profile_image_url,"@"+b.user.screen_name+
" retweeted you!",b.retweeted_status.text)}}else if(d.isJSON())d.evalJSON(true)?console.log("unknown error, output: "+d):console.log("json could not be properly evaluated, skipping");else console.log("not json, skipping")});socket.on("disconnect",function(){$("new-tweet").disable();setTimeout(function(){window.location="/"},5E3)})}
...
function z_engine_dropped_image(a){if(a.size<=max_file_size*1048576)switch(a.type){case "application/pdf":case "image/apng":case "image/bmp":case "image/gif":case "image/jpeg":case "image/jpg":case "image/png":case "image/tiff":var c=new FormData;c.append("image",a);c.append("key",imgur_key);a=new XMLHttpRequest;a.onreadystatechange=function(){this.readyState==3&&$("image").setStyle("border-color: yellow;");if(this.readyState==4&&this.status==200){var b=this.responseText.evalJSON(true).upload.links.original,
d=$("new-tweet").getValue();if(d.length>0)var e=d+" "+b;else if(d.length==0)e=b;$("new-tweet").setValue(e);$("image").setStyle("border-color: green;");setTimeout(function(){$("image").setStyle("border-color: #ddd;")},1500)}if(this.readyState==4&&this.status!=200){$("image").setStyle("border-color: red;");setTimeout(function(){$("image").setStyle("border-color: #ddd;")},1500)}};a.open("POST","http://api.imgur.com/2/upload.json",true);a.send(c)}}
...
function z_engine_geolocation_error(a){switch(a.code){case 0:case 1:case 2:case 3:longit=latit=false}}function z_engine_get_klout(a,c){socket.emit("message",{klout:a,id_str:c})}function z_engine_logout(){socket.disconnect();$("new-tweet").disable();$("new-tweet").setValue("see ya!");store.remove("account");store.remove("friends");store.remove("screen_name");store.remove("user_id");window.location="/oauth/logout"}
...
function z_engine_shorten_urls(){var a=$("new-tweet").getValue();a.length>0&&a.replace(/((https?\:\/\/)|(www\.))([^ ]+)/g,function(c){socket.emit("message",{shorten:c})})}function z_engine_stream_queue(){if(content_queued.length>0){var a=content_queued.shift();if(typeof a!=="undefined")if(a.isJSON()){a=a.evalJSON(true);z_engine_tweet(a,"home")}}}
function z_engine_tweet(a,c){if(c!="dms")if(a.retweeted_status){b=a.retweeted_status.user.screen_name;d=a.user.screen_name;e=a.retweeted_status.user.profile_image_url;j=(new Date(a.retweeted_status.created_at)).toLocaleString().replace(/GMT.+/,"");f=a.retweeted_status.user.description;i=a.retweeted_status.entities;h=a.retweeted_status.favorited;v=a.retweeted_status.user.followers_count;p=a.retweeted_status.user.friends_count;g=a.retweeted_status.id_str;k=a.retweeted_status.user.location;A=a.retweeted_status.user["protected"];
q=a.retweeted_status.place;l=a.retweeted_status.in_reply_to_screen_name;o=a.retweeted_status.in_reply_to_status_id_str;n=true;D=a.retweeted_status.source.stripTags();w=a.retweeted_status.text;x=a.retweeted_status.user.statuses_count;B=a.retweeted_status.user.id;r=a.retweeted_status.user.verified}else var b=a.user.screen_name,d=false,e=a.user.profile_image_url,j=(new Date(a.created_at)).toLocaleString().replace(/GMT.+/,""),f=a.user.description,i=a.entities,h=a.favorited,v=a.user.followers_count,p=
a.user.friends_count,g=a.id_str,k=a.user.location,A=a.user["protected"],q=a.place,l=a.in_reply_to_screen_name,o=a.in_reply_to_status_id_str,n=false,D=a.source.stripTags(),w=a.text,x=a.user.statuses_count,B=a.user.id,r=a.user.verified;else{b=a.sender.screen_name;e=a.sender.profile_image_url;j=(new Date(a.created_at)).toLocaleString().replace(/GMT.+/,"");f=a.sender.description;i=false;v=a.sender.followers_count;p=a.sender.friends_count;g=a.id_str;k=a.sender.location;A=false;l=a.recipient.screen_name;
n=false;w=a.text;x=a.sender.statuses_count;B=a.sender_id;r=a.sender.verified}var G=false,H=false,I=false,J=false;c!="dms"&&$w(store.get("client_blocks")).uniq().each(function(t){if(t==D)G=true});if(i){typeof i.hashtags=="object"&&$w(store.get("hashtag_blocks")).uniq().each(function(t){i.hashtags.uniq().each(function(E){if(t==E)H=true})});typeof i.user_mentions=="object"&&$w(store.get("mention_blocks")).uniq().each(function(t){i.user_mentions.uniq().each(function(E){if(t==E)I=true})})}$w(store.get("user_blocks")).uniq().each(function(t){if(t==
b)J=true});if(!content[g]&&!G&&!H&&!I&&!J){var u="";if(f!=null)u+=f+"<br /><br />";if(k!=null)u+="location: "+k+"<br />";u+="tweets: <strong>"+x+"</strong><br />";u+="following: <strong>"+p+"</strong><br />";u+="followers: <strong>"+v+"</strong>";ids[tid]=g;content[g]=true;var C=new Element("br");if(i){p=z_engine_tweet_mentioned(i);f=z_engine_tweet_mentioned_string(i)}else f=p=false;k=new Element("li",{id:"comment-"+g,"class":"comment-parent",style:"display: none; opacity: 0;"});x=new Element("div",
{"class":"comment-profile-wrapper left"});var y=new Element("div",{"class":"comment-gravatar"});v=new Element("img",{id:"av-"+g,src:e,style:"height: 50px; width: 50px; cursor: pointer;",alt:""});y.insert(v);x.insert(y);y=new Element("div",{id:"comment-"+g+"content","class":"comment-content-wrapper right"});var z=b!=screen_name?p?new Element("div",{"class":"comment-body-mentioned"}):new Element("div",{"class":"comment-body"}):new Element("div",{"class":"comment-body-me"}),m=new Element("div",{"class":"comment-arrow"});
z.insert(m);var s=new Element("div",{"class":"post-date"});m=new Element("div",{"class":"left",id:"left-"+g});var K=new Element("a",{target:"_blank",href:"http://twitter.com/"+b});K.update("@"+b+" ");var L=new Element("span");L.update("wrote this ");if(c!="dms")var M=new Element("a",{target:"_blank",id:"comment-"+g+"-relative-date",href:"http://twitter.com/"+b+"/status/"+g});var F=new Element("time",{datetime:j,"class":c});F.update(relative_time(j));c!="dms"&&M.insert(F);m.insert(K);m.insert({bottom:L});
if(c!="dms"){m.insert({bottom:M});if(o){j=new Element("span");j.update(" in reply to ");o=new Element("a",{target:"_blank",href:"http://twitter.com/"+l+"/status/"+o});o.update(l+" ");m.insert(j);m.insert({bottom:o})}l=new Element("span",{"class":"via"});l.update(" via "+D);m.insert({bottom:l})}else{m.insert({bottom:F});if(l){j=new Element("span");j.update(" in reply to ");o=new Element("a",{target:"_blank",href:"http://twitter.com/"+l});o.update(l+" ");m.insert(j);m.insert({bottom:o})}}if(r){r=new Element("span");
r.update(" ");l=new Element("img",{src:"img/ver.png",alt:"",title:"verified user"});r.insert({bottom:l});m.insert({bottom:r})}s.insert(m);r=new Element("div",{"class":"right"});if(n){n=new Element("span",{"class":"rtd"});n.update(" ");l=new Element("img",{src:"img/rtd2.png",alt:""});o=new Element("a",{target:"_blank",href:"http://twitter.com/"+d});o.update(d);n.insert({top:l});n.insert({bottom:o});r.insert({bottom:n})}if(c!="dms"&&q&&q.full_name){d=new Element("span",{"class":"place"});q=new Element("a",
{target:"_blank",href:"http://maps.google.com?q="+q.full_name});n=new Element("img",{src:"img/plc.png",alt:""});q.update(n);d.update(q);r.insert({bottom:d})}s.insert({bottom:r});z.insert({bottom:s});d=new Element("div",{"class":"clearer"});d.update("&nbsp;");z.insert({bottom:d});n=new Element("div",{"class":"comment-text"});s=new Element("p",{id:"comment-"+g+"-text"});q=new Element("div",{id:"right-"+g,"class":"right"});s.insert(C);s.insert({bottom:q});C=new Element("div",{id:"text-"+g});C.update(z_engine_parse_tweet(w));
s.insert({bottom:C});n.insert(s);z.insert({bottom:n});y.insert(z);k.insert(x);k.insert({bottom:y});k.insert({bottom:d});new Element.extend(q);new Element.extend(k);switch(c){case "dms":if(b!=screen_name)$("dms-inbox-timeline").insert({top:k});else b==screen_name&&$("dms-outbox-timeline").insert({top:k});break;case "mentions":$("mentions-timeline").insert({bottom:k});break;case "home":$("home-timeline").insert({top:k})}if(p&&c!="mentions"&&b!=screen_name&&!a.retweeted_status){p=cloneNodeWithEvents(k);
p.setAttribute("id","comment-"+g+"-mentioned");m.setAttribute("id","left-"+g+"-mentioned");q.setAttribute("id","right-"+g+"-mentioned");v.setAttribute("id","av-"+g+"-mentioned");z_engine_tweet_buttons("mentions",g,b,B,w,A,h,f,u);new Element.extend(p);$("mentions-timeline").insert({top:p});z_engine_notification(e,"@"+b+" mentioned you!",w);new S2.FX.Parallel([new Effect.Appear("comment-"+g+"-mentioned",{duration:1.25,mode:"relative",transition:"easeOutSine"}),new Effect.BlindDown("comment-"+g+"-mentioned",
{duration:0.7,mode:"relative",transition:"easeOutSine"})],{duration:1.5})}z_engine_tweet_buttons(c,g,b,B,w,A,h,f,u);new S2.FX.Parallel([new Effect.Appear("comment-"+g,{duration:1.25,mode:"relative",transition:"easeOutSine"}),new Effect.BlindDown("comment-"+g,{duration:0.7,mode:"relative",transition:"easeOutSine"})],{duration:1.5})}tid++}
...
new Element("img",{onclick:'z_engine_get_klout("'+b+'", "'+c+'");',src:"img/klt.png",id:"klout-"+c,alt:"",title:"click to get this users klout score"});f.insert({top:b});$("left-"+c).insert({top:f});new Element.extend(b)}$("av-"+c)&&$("av-"+c).addTip(z_engine_parse_tweet(h),{className:"user",hideDelay:3,showOn:"click",offset:[11,0],stem:true,target:true,targetJoint:["right","middle"],tipJoint:["left","middle"]});break;case "mentions":if(b!=screen_name){a=new Element("img",{src:"img/rep.png",onclick:'z_engine_reply("'+
b+'", "'+c+'-mentioned", "'+i+'");',id:"reply-"+c+"-mentioned",alt:""});if(j)e=new Element("img",{src:"img/lock.png",style:"cursor: default;",alt:""});else{e=new Element("img",{src:"img/rt.png",onclick:'z_engine_retweet("'+c+'", "'+b+'", "'+escape_string(e)+'");',id:"rt-"+c+"-mentioned",alt:""});new Element.extend(e)}f=f?new Element("img",{src:"img/favd.png",onclick:'z_engine_unfavorite("'+c+'");',id:"fave-"+c+"-mentioned",alt:""}):new Element("img",{src:"img/fav.png",onclick:'z_engine_favorite("'+
c+'");',id:"fave-"+c+"-mentioned",alt:""});new Element.extend(a);new Element.extend(f);if($("right-"+c)){$("right-"+c).insert(a);$("right-"+c).insert({bottom:e});$("right-"+c).insert({bottom:f})}if($("right-"+c+"-mentioned")){$("right-"+c+"-mentioned").insert(a);$("right-"+c+"-mentioned").insert({bottom:e});$("right-"+c+"-mentioned").insert({bottom:f})}}else{f=new Element("img",{onclick:'z_engine_destroy("'+c+'", "tweet");',src:"img/del.png",id:"del-"+c+"-mentioned",alt:""});$("right-"+c)&&$("right-"+
c).insert({bottom:f});$("right-"+c+"-mentioned")&&$("right-"+c+"-mentioned").insert({bottom:f});new Element.extend(f)}$("av-"+c)&&$("av-"+c).addTip(h,{className:"user",offset:[11,0],stem:true,target:true,targetJoint:["right","middle"],tipJoint:["left","middle"]});$("av-"+c+"-mentioned")&&$("av-"+c+"-mentioned").addTip(h,{className:"user",offset:[11,0],stem:true,target:true,targetJoint:["right","middle"],tipJoint:["left","middle"]});break;case "dms":if(b!=screen_name){a=new Element("img",{onclick:'z_engine_reply_dm("'+
d+'", "'+b+'");',src:"img/rep.png",id:"reply-"+c,alt:""});new Element.extend(a);$("right-"+c).insert(a)}f=new Element("img",{onclick:'z_engine_destroy("'+c+'", "dm");',src:"img/del.png",id:"del-"+c,alt:""});new Element.extend(f);$("right-"+c).insert({bottom:f})}}function z_engine_tweet_mentioned(a){var c=false;a.user_mentions.each(function(b){if(b.screen_name==screen_name)c=true});return c}
