...
"dms");a.direct_message.sender.screen_name!=screen_name&&z_engine_notification(a.direct_message.sender.profile_image_url,"@"+a.direct_message.sender.screen_name+" sent a direct message",a.direct_message.text)}else if(a.dms){dms_loaded++;a.dms.each(function(h){z_engine_tweet(h,"dms")});switch(dms_loaded){case 1:$("loading-inbox").fade();$("loading-outbox").appear();break;case 2:$("loading-outbox").fade()}}else if(a.event)switch(a.event){case "block":$w(store.get("mention_blocks")).uniq();c=$w(store.get("user_blocks")).uniq();
var k=false,f=false;d.each(function(h){if(h==a.target.screen_name)k=true});c.each(function(h){if(h==a.target.screen_name)f=true});k||store.set("mention_blocks",current_blocks.join(" ")+" "+a.target.screen_name);f||store.set("user_blocks",current_blocks.join(" ")+" "+a.target.screen_name);break;case "unblock":d=$w(store.get("user_blocks")).uniq();c=$w(store.get("user_blocks")).uniq();var i="",l="";d.each(function(h){if(h!=a.target.screen_name)i+=h+" "});c.each(function(h){if(h!=a.target.screen_name)l+=
h+" "});store.set("mention_blocks",i);store.set("user_blocks",l);break;case "favorite":a.source.screen_name!=screen_name&&z_engine_notification(a.source.profile_image_url,"@"+a.source.screen_name+" faved your tweet!",a.target_object.text);break;case "unfavorite":break;case "follow":a.source.screen_name!=screen_name&&z_engine_notification(a.source.profile_image_url,"@"+a.source.screen_name+" is following you!",a.source.description);break;case "list_member_added":a.source.screen_name!=screen_name&&
...
case "scrub_geo":break;case "user_update":break;default:console.log(JSON.stringify(a.event));console.log(JSON.stringify(a))}else if(a.friends){d=a.friends.join(" ");store.set("friends",d)}else if(a.home){a.home.each(function(h){z_engine_tweet(h,"home")});$("loading-home").fade();$("loading-mentions").appear();socket.emit("message",{fetch:"userstream"})}else if(a.info){screen_name=a.info.screen_name;user_id=a.info.user_id;store.set("screen_name",screen_name);store.set("user_id",user_id)}else if(a.klout){c=
a.klout;d=a.id_str;if(c!="error"){z_engine_set_klout(c.users,d);klout[c.users[0].twitter_id]=JSON.stringify(c.users)}else if($("klout-"+d)){$("klout-"+d).setStyle("cursor: default;");$("klout-"+d).setAttribute("onclick","");$("klout-"+d).addTip("<big><strong>?</strong></big>",kloutinfo_params)}}else if(a.mentions){a.mentions.each(function(h){z_engine_tweet(h,"mentions")});$("loading-mentions").fade();$("loading-inbox").appear()}else if(a.rates)rates=JSON.stringify(a.rates);else if(a.retweet_info){c=
a.retweet_info;d=c.retweeted_status.id_str;if($("comment-"+d)){if($("rt-"+d)){$("rt-"+d).writeAttribute("src","img/rtd.png");$("rt-"+d).writeAttribute("onclick","z_engine_destroy('"+c.id_str+"','rt');")}if($("rt-"+d+"-mentioned")){$("rt-"+d+"-mentioned").writeAttribute("src","img/rtd.png");$("rt-"+d+"-mentioned").writeAttribute("onclick","z_engine_destroy('"+c.id_str+"','rt');")}if($("rt-"+d+"-threaded")){$("rt-"+d+"-threaded").writeAttribute("src","img/rtd.png");$("rt-"+d+"-threaded").writeAttribute("onclick",
"z_engine_destroy('"+c.id_str+"','rt');")}}}else if(a.server_event){console.log(JSON.stringify(a));switch(a.server_event){case "end":$("new-tweet").disable();z_engine_notification("","notice!","lost connection to the userstream, reconnecting...");break;case "error":$("new-tweet").disable();z_engine_notification("","notice!","error occurred, reconnecting...")}}else if(a.shorten){d=$("new-tweet").getValue().replace(a.original,a.shorten);$("new-tweet").setValue(d)}else if(a.show)z_engine_tweet(a.show,
"threaded");else if(a.text&&a.user&&a.created_at){content_queued.push(JSON.stringify(a));if(paused){$("paused-count").update("("+pttid+")");pttid++}a.retweeted_status&&a.retweeted_status.user.screen_name==screen_name&&a.user.screen_name!=screen_name&&z_engine_notification(a.user.profile_image_url,"@"+a.user.screen_name+" retweeted you!",a.retweeted_status.text)}}else if(c.isJSON())c.evalJSON(true)?console.log("unknown error, output: "+c):console.log("json could not be properly evaluated, skipping");
else console.log("not json, skipping")});socket.on("disconnect",function(){$("new-tweet").disable()})}function z_engine_check_ratelimit(){socket.emit("message",{fetch:"rates"})}
...
function z_engine_retweet(b){socket.emit("message",{retweet:{status:{id_str:b}}})}function z_engine_retweet_comment(b,a,c){reply_id=b;b=(new Element("textarea")).update(escape_string(c.replace(/</g,"&lt;").replace(/>/g,"&gt;")));$("new-tweet").setValue("RT @"+a+" "+b.getValue());$("new-tweet").focus()}
...
F)J=true})})}$w(store.get("user_blocks")).uniq().each(function(w){if(w==c)K=true});if(!s&&!H&&!I&&!J&&!K){content_stored[e]=JSON.stringify(b);s="";if(i!=null)s+=i+"<br /><br />";if(n!=null)s+="location: "+n+"<br />";s+="tweets: <strong>"+y+"</strong><br />";s+="following: <strong>"+j+"</strong><br />";s+="followers: <strong>"+g+"</strong>";if(l){n=z_engine_tweet_mentioned(l);i=z_engine_tweet_mentioned_string(l)}else i=n=false;var D=new Element("br");g=new Element("div",{id:"comment-"+e,"class":"comment-parent"});
y=new Element("div",{"class":"comment-profile-wrapper left"});var B=new Element("div",{"class":"comment-gravatar"});j=new Element("img",{id:"av-"+e,src:k,style:"height: 50px; width: 50px; cursor: pointer;",alt:""});B.insert(j);y.insert(B);B=new Element("div",{id:"comment-"+e+"content","class":"comment-content-wrapper right"});var C=c!=screen_name?n?new Element("div",{"class":"comment-body-mentioned"}):new Element("div",{"class":"comment-body"}):new Element("div",{"class":"comment-body-me"}),m=new Element("div",
{"class":"comment-arrow"});C.insert(m);var t=new Element("div",{"class":"post-date"});m=new Element("div",{"class":"left",id:"left-"+e});var L=new Element("a",{target:"_blank",href:"http://twitter.com/"+c});L.update("@"+c+" ");var M=new Element("span");M.update("wrote this ");if(a!="dms")var u=new Element("a",{target:"_blank",id:"comment-"+e+"-relative-date",href:"http://twitter.com/"+c+"/status/"+e});var G=new Element("time",{datetime:f,"class":a});G.update(relative_time(f));a!="dms"&&u.insert(G);
m.insert(L);m.insert({bottom:M});if(a!="dms"){m.insert({bottom:u});if(x){f=new Element("span");f.update(" in reply to ");u=a=="threaded"?new Element("a",{target:"_blank",href:"http://twitter.com/"+o+"/status/"+x}):new Element("span",{onclick:'z_engine_threaded("'+e+'","'+x+'");',style:"cursor: pointer;"});u.update(o+" ");m.insert(f);m.insert({bottom:u})}o=new Element("span",{"class":"via"});o.update(" via "+E);m.insert({bottom:o})}else{m.insert({bottom:G});if(o){f=new Element("span");f.update(" in reply to ");
u=new Element("a",{target:"_blank",href:"http://twitter.com/"+o});u.update(o+" ");m.insert(f);m.insert({bottom:u})}}if(r){r=new Element("span");r.update(" ");o=new Element("img",{src:"img/ver.png",alt:"",title:"verified user"});r.insert({bottom:o});m.insert({bottom:r})}t.insert(m);r=new Element("div",{"class":"right"});if(p){p=new Element("span",{"class":"rtd"});p.update(" ");o=new Element("img",{src:"img/rtd2.png",alt:""});f=new Element("a",{target:"_blank",href:"http://twitter.com/"+d});f.update(d);
p.insert({top:o});p.insert({bottom:f});r.insert({bottom:p})}if(a!="dms"&&q&&q.full_name){d=new Element("span",{"class":"place"});q=new Element("a",{target:"_blank",href:"http://maps.google.com?q="+q.full_name});p=new Element("img",{src:"img/plc.png",alt:""});q.update(p);d.update(q);r.insert({bottom:d})}t.insert({bottom:r});C.insert({bottom:t});d=new Element("div",{"class":"clearer"});d.update("&nbsp;");C.insert({bottom:d});p=new Element("div",{"class":"comment-text"});t=new Element("p",{id:"comment-"+
e+"-text"});q=new Element("div",{id:"right-"+e,"class":"right"});t.insert(D);t.insert({bottom:q});D=new Element("div",{id:"text-"+e});D.update(z_engine_parse_tweet(v));t.insert({bottom:D});p.insert(t);C.insert({bottom:p});B.insert(C);g.insert(y);g.insert({bottom:B});g.insert({bottom:d});new Element.extend(j);new Element.extend(m);new Element.extend(q);new Element.extend(g);switch(a){case "dms":if(c!=screen_name)$("dms-inbox-timeline").insert({top:g});else c==screen_name&&$("dms-outbox-timeline").insert({top:g});
break;case "home":$("home-timeline").insert({top:g});break;case "mentions":$("mentions-timeline").insert({bottom:g})}if(n&&a!="mentions"&&a!="threaded"&&c!=screen_name&&!b.retweeted_status){n=cloneNodeWithEvents(g);n.setAttribute("id","comment-"+e+"-mentioned");m.setAttribute("id","left-"+e+"-mentioned");q.setAttribute("id","right-"+e+"-mentioned");j.setAttribute("id","av-"+e+"-mentioned");new Element.extend(n);$("mentions-timeline").insert({top:n});z_engine_tweet_buttons("mentions",e,c,A,v,z,h,i,
s);z_engine_notification(k,"@"+c+" mentioned you!",v);z_engine_fade_down("comment-"+e+"-mentioned")}if(a=="threaded"){m.setAttribute("id","left-"+e+"-threaded");q.setAttribute("id","right-"+e+"-threaded");j.setAttribute("id","av-"+e+"-threaded");k=cloneNodeWithEvents(g);k.setAttribute("id","comment-"+e+"-threaded");new Element.extend(k);$("threaded-timeline").insert({bottom:k});z_engine_tweet_buttons("threaded",e,c,A,v,z,h,i,s);z_engine_fade_down("comment-"+e+"-threaded")}$("comment-"+e)&&z_engine_tweet_buttons(a,
e,c,A,v,z,h,i,s)}a=="threaded"&&x&&z_engine_threaded(false,x)}
