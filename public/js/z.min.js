...
function z_engine_tweet(a,b){var c=$w(b),g=c[0];c=c[1];if(g!="dms")if(a.retweeted_status){h=a.retweeted_status.user.screen_name;f=a.user.screen_name;j=a.retweeted_status.user.profile_image_url;e=BrowserDetect.browser=="Opera"?a.retweeted_status.created_at:(new Date(a.retweeted_status.created_at)).toLocaleString().replace(/GMT.+/,"");i=a.retweeted_status.entities;n=a.retweeted_status.favorited;d=a.retweeted_status.id_str;m=a.retweeted_status.user["protected"];l=a.retweeted_status.place;o=a.retweeted_status.in_reply_to_screen_name;
v=a.retweeted_status.in_reply_to_status_id_str;k=a.retweeted_status.retweet_count-1;t=true;E=a.retweeted_status.source.stripTags();z=a.retweeted_status.text;C=a.retweeted_status.user.id;p=a.retweeted_status.user.verified}else{var h=a.user.screen_name,f=false,j=a.user.profile_image_url,e=BrowserDetect.browser=="Opera"?a.created_at:(new Date(a.created_at)).toLocaleString().replace(/GMT.+/,"");e=a.created_at;var i=a.entities,n=a.favorited,d=a.id_str,m=a.user["protected"],l=a.place,o=a.in_reply_to_screen_name,
v=a.in_reply_to_status_id_str,k=a.retweet_count-1,t=false,E=a.source.stripTags(),z=a.text,C=a.user.id,p=a.user.verified}else{h=a.sender.screen_name;j=a.sender.profile_image_url;e=(new Date(a.created_at)).toLocaleString().replace(/GMT.+/,"");i=false;d=a.id_str;m=false;o=a.recipient.screen_name;k=0;t=false;z=a.text;C=a.sender_id;p=a.sender.verified}var J=false,K=false,L=false,M=false,w=false;if(g!="threaded"&&content_stored[d])w=true;g!="dms"&&$w(store.get("client_blocks")).uniq().each(function(A){if(A.replace("'",
"")==E)J=true});if(i){typeof i.hashtags=="object"&&$w(store.get("hashtag_blocks")).uniq().each(function(A){i.hashtags.uniq().each(function(F){if(A==F)K=true})});typeof i.user_mentions=="object"&&$w(store.get("mention_blocks")).uniq().each(function(A){i.user_mentions.uniq().each(function(F){if(A==F)L=true})})}$w(store.get("user_blocks")).uniq().each(function(A){if(A==h)M=true});if(!w&&!J&&!K&&!L&&!M){(new Image).src=j;content_stored[d]=Object.toJSON(a);if(i){G=z_engine_tweet_mentioned(i);w=z_engine_tweet_mentioned_string(i)}else{var G=
...
o+"/status/"+v}):new Element("span",{onclick:'z_engine_threaded("'+d+'","'+v+'");',style:"cursor: pointer;"});e.update("@"+o+" ");q.insert(s);q.insert({bottom:e})}o=new Element("span",{"class":"via"});o.update(translation.via+" "+E);q.insert({bottom:o})}else{q.insert({bottom:I});if(o){s=new Element("span");s.update(" in reply to ");e=new Element("a",{target:"_blank",href:"http://twitter.com/"+o});e.update(o+" ");q.insert(s);q.insert({bottom:e})}}if(p){p=new Element("span");p.update(" ");o=new Element("img",
{src:"/img/ver.png",alt:"",title:"verified user"});p.insert({bottom:o});q.insert({bottom:p})}y.insert(q);p=new Element("div",{"class":"right"});if(g!="dms"&&l&&l.full_name){o=new Element("span",{"class":"place",id:"place-"+d});s=new Element("a",{target:"_blank",href:"http://maps.google.com?q="+l.full_name});l=new Element("img",{src:"/img/plc.png",alt:"",title:l.full_name});s.update(l);o.update(s);p.insert({bottom:o})}if(t){k=k>0?translation.retweeted_by+" "+f+" + "+k+" "+translation.others+"!":translation.retweeted_by+
" "+f+"!";l=new Element("span",{"class":"rtd"});l.update(" ");k=new Element("img",{src:"/img/rtd2.png",alt:"",title:k});o=new Element("a",{target:"_blank",href:"http://twitter.com/"+f});o.update(f);l.insert({top:k});l.insert({bottom:o});p.insert({bottom:l})}y.insert({bottom:p});r.insert({bottom:y});l=new Element("div",{"class":"clearer"});l.update("&nbsp;");r.insert({bottom:l});k=new Element("div",{"class":"comment-text"});p=new Element("p",{id:"comment-"+d+"-text"});y=new Element("div",{id:"right-"+
...
else h==screen_name&&$("dms-outbox-timeline").insert(u);break;case "home":$("home-timeline").insert(u);break;case "mentions":$("mentions-timeline").insert(u)}if(G&&g!="mentions"&&g!="threaded"&&h!=screen_name&&!a.retweeted_status){u=cloneNodeWithEvents(x);u.writeAttribute("id","comment-"+d+"-mentioned");r.writeAttribute("id","comment-body-"+d+"-mentioned");q.writeAttribute("id","left-"+d+"-mentioned");y.writeAttribute("id","right-"+d+"-mentioned");H.writeAttribute("id","av-"+d+"-mentioned");new Element.extend(u);
switch(c){case "bottom":var B={bottom:u};break;case "top":B={top:u}}$("mentions-timeline").insert(B);z_engine_tweet_buttons("mentions",d,h,f,C,z,m,n,t,w);z_engine_fade_down("comment-"+d+"-mentioned");z_engine_notification(j,"@"+h+" mentioned you!",z)}if(g=="threaded"){r.writeAttribute("id","comment-body-"+d+"-threaded");q.writeAttribute("id","left-"+d+"-threaded");y.writeAttribute("id","right-"+d+"-threaded");H.writeAttribute("id","av-"+d+"-threaded");B=cloneNodeWithEvents(x);B.writeAttribute("id",
...
b,c,g,h,n,f,e,i,j,a);break;case "home":if(c!=screen_name){d=new Element("img",{src:"/img/rep.png",onclick:'z_engine_reply("'+c+'", "'+b+'", "'+n+'");',title:translation.reply,id:"reply-"+b,alt:""});new Element.extend(d);if(j)m=new Element("img",{src:"/img/lock.png",style:"cursor: default;",alt:""});else{var m=new Element("img",{src:"/img/rt.png",onclick:'z_engine_retweet("'+b+'");',title:translation.retweet,id:"rt-"+b,alt:""});new Element.extend(m)}var l=e?new Element("img",{src:"/img/favd.png",onclick:'z_engine_unfavorite("'+
b+'");',title:translation.unfavorite,id:"fave-"+b,alt:""}):new Element("img",{src:"/img/fav.png",onclick:'z_engine_favorite("'+b+'");',title:translation.favorite,id:"fave-"+b,alt:""});$("right-"+b).update();$("right-"+b).insert(d);$("right-"+b).insert({bottom:m});$("right-"+b).insert({bottom:l})}else{d=new Element("img",{onclick:'z_engine_destroy("'+b+'", "tweet");',title:translation.destroy,src:"/img/del.png",id:"del-"+b,alt:""});new Element.extend(d);$("right-"+b).update();$("right-"+b).insert({bottom:d})}z_engine_tweet_right_click(b,
"comment-"+b,c,g,h,n,f,e,i,j,a);break;case "mentions":if(c!=screen_name){d=new Element("img",{src:"/img/rep.png",onclick:'z_engine_reply("'+c+'", "'+b+'-mentioned", "'+n+'");',title:translation.reply,id:"reply-"+b+"-mentioned",alt:""});new Element.extend(d);if(j)m=new Element("img",{src:"/img/lock.png",style:"cursor: default;",alt:""});else{m=new Element("img",{src:"/img/rt.png",onclick:'z_engine_retweet("'+b+'");',title:translation.retweet,id:"rt-"+b+"-mentioned",alt:""});new Element.extend(m)}l=
e?new Element("img",{src:"/img/favd.png",onclick:'z_engine_unfavorite("'+b+'");',title:translation.unfavorite,id:"fave-"+b+"-mentioned",alt:""}):new Element("img",{src:"/img/fav.png",onclick:'z_engine_favorite("'+b+'");',title:translation.favorite,id:"fave-"+b+"-mentioned",alt:""});if($("right-"+b)){$("right-"+b).update();$("right-"+b).insert(d);$("right-"+b).insert({bottom:m});$("right-"+b).insert({bottom:l})}if($("right-"+b+"-mentioned")){$("right-"+b+"-mentioned").update();$("right-"+b+"-mentioned").insert(d);
$("right-"+b+"-mentioned").insert({bottom:m});$("right-"+b+"-mentioned").insert({bottom:l})}}else{d=new Element("img",{onclick:'z_engine_destroy("'+b+'", "tweet");',title:translation.destroy,src:"/img/del.png",id:"del-"+b+"-mentioned",alt:""});new Element.extend(d);if($("right-"+b)){$("right-"+b).update();$("right-"+b).insert({bottom:d})}if($("right-"+b+"-mentioned")){$("right-"+b+"-mentioned").update();$("right-"+b+"-mentioned").insert({bottom:d})}}$("comment-"+b)&&z_engine_tweet_right_click(b,"comment-"+
b,c,g,h,n,f,e,i,j,a);$("comment-"+b+"-mentioned")&&z_engine_tweet_right_click(b,"comment-"+b+"-mentioned",c,g,h,n,f,e,i,j,a);break;case "threaded":if(c!=screen_name){d=new Element("img",{src:"/img/rep.png",onclick:'z_engine_reply("'+c+'", "'+b+'", "'+n+'");',title:translation.reply,id:"reply-"+b+"-threaded",alt:""});new Element.extend(d);if(j)m=new Element("img",{src:"/img/lock.png",style:"cursor: default;",alt:""});else{m=new Element("img",{src:"/img/rt.png",onclick:'z_engine_retweet("'+b+'");',
id:"rt-"+b+"-threaded",title:translation.retweet,alt:""});new Element.extend(m)}l=e?new Element("img",{src:"/img/favd.png",onclick:'z_engine_unfavorite("'+b+'");',title:translation.unfavorite,id:"fave-"+b+"-threaded",alt:""}):new Element("img",{src:"/img/fav.png",onclick:'z_engine_favorite("'+b+'");',title:translation.favorite,id:"fave-"+b+"-threaded",alt:""});if($("right-"+b+"-threaded")){$("right-"+b+"-threaded").update();$("right-"+b+"-threaded").insert(d);$("right-"+b+"-threaded").insert({bottom:m});
...
function z_engine_tweet_right_click(a,b,c,g,h,f,j,e,i,n,d){var m=false,l=true,o=false,v=false;if(c==screen_name){m=true;l=false}i||(o=true);if(i&&g!=screen_name){o=true;i=false}else if(i&&g==screen_name)v=true;var k=new Control.ContextMenu(b,{animation:false});switch(d){case "home":case "mentions":case "threaded":if(m)k.addItem({label:'<img src="/img/del.png" alt="" />',callback:function(){z_engine_destroy(a,"tweet")}});else{k.addItem({label:'<img src="/img/rep.png" alt="" />',callback:function(){z_engine_reply(c,
a,f)}});if(n)k.addItem({label:'<img src="/img/lock.png" alt="" />',enabled:false});else{k.addItem({label:function(){return'<img src="/img/rt.png" alt="" />'},condition:function(){return o},callback:function(){z_engine_retweet(a);k.destroy();new PeriodicalExecuter(function(t){if(content_rts_stored[a]&&content_rts_stored[a].isJSON()){t.stop();t=content_rts_stored[a].evalJSON(true).id_str;z_engine_tweet_right_click(t,b,c,screen_name,h,f,j,e,true,n,d)}},0.25)}});k.addItem({label:function(){return"<span>RT</span>"},
condition:function(){return o},callback:function(){z_engine_retweet_comment(a,c,j)}});k.addItem({label:function(){return'<img src="/img/rtd.png" alt="" />'},condition:function(){return v},callback:function(){z_engine_destroy(a,"rt");k.close();k.destroy();z_engine_tweet_right_click(a,b,c,screen_name,h,f,j,e,false,n,d)}})}e?k.addItem({label:'<img src="/img/favd.png" alt="" />',callback:function(){z_engine_unfavorite(a);k.close();k.destroy();z_engine_tweet_right_click(a,b,c,g,h,f,j,false,i,n,d)}}):k.addItem({label:'<img src="/img/fav.png" alt="" />',
callback:function(){z_engine_favorite(a);k.close();k.destroy();z_engine_tweet_right_click(a,b,c,g,h,f,j,true,i,n,d)}})}break;case "dms":k.addItem({label:function(){return'<img src="/img/rep.png" alt="" />'},condition:function(){return l},callback:function(){z_engine_reply_dm(h,c)}});k.addItem({label:'<img src="/img/del.png" alt="" />',callback:function(){z_engine_destroy(a,"dm")}})}}
